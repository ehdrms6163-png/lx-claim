...
function openCADetail(id){
  ...
  $('ca-detail-content').innerHTML=`
    ...
    ${(()=>{
      const lm=c.minorRef?minorClaims.find(x=>x.id===c.minorRef):null;
      const lc=c.claimRef?claims.find(x=>x.id===c.claimRef):(!lm?.claimRef?null:claims.find(x=>x.id===lm.claimRef));
      const ls=lc?lawsuits.filter(x=>x.claimRef===lc.id):[];
      const parts=[];
      if(lm)parts.push(`<div style="padding:10px 12px;background:#EAF3DE;border:0.5px solid #C0DD97;border-radius:var(--r-md);margin-bottom:7px;">
        <div style="display:flex;align-items:center;gap:7px;margin-bottom:4px;">
          <span style="font-size:11px;font-weight:600;color:var(--green);">소액클레임</span>
          <span style="font-family:var(--mono);font-size:11px;color:var(--green);">${lm.id}</span>
          <span class="bdg b-접수" style="font-size:10px;">${lm.status}</span>
          <button class="btn sm" style="margin-left:auto;font-size:11px;" data-goto-minor="${lm.id}">바로가기 \u2192</button>
        </div>
        <div style="font-size:12px;color:var(--tx2);">${lm.name||'-'} / ${lm.type||'-'} / ${lm.adate||lm.idate||'-'}</div>
        <div style="font-size:12px;color:var(--tx2);">${lm.logistics||''} ${lm.tech?'· 기사: '+lm.tech:''}</div>
      </div>`);
      if(lc)parts.push(`<div style="padding:10px 12px;background:var(--bg2);border:0.5px solid var(--bd2);border-radius:var(--r-md);margin-bottom:7px;">
        <div style="display:flex;align-items:center;gap:7px;margin-bottom:4px;">
          <span style="font-size:11px;font-weight:600;color:var(--tx2);">보험 클레임</span>
          <span style="font-family:var(--mono);font-size:11px;color:var(--blue);">${lc.id}</span>
          <span class="bdg b-${lc.status}" style="font-size:10px;">${lc.status}</span>
          <button class="btn sm" style="margin-left:auto;font-size:11px;" data-goto-claim="${lc.id}">바로가기 \u2192</button>
        </div>
        <div style="font-size:12px;color:var(--tx2);">${lc.name||'-'} / ${lc.type||'-'}</div>
      </div>`);
      ls.forEach(s=>parts.push(`<div style="padding:10px 12px;background:#FCEBEB;border:0.5px solid #F7C1C1;border-radius:var(--r-md);margin-bottom:7px;">
        <div style="display:flex;align-items:center;gap:7px;margin-bottom:4px;">
          <span style="font-size:11px;font-weight:600;color:#922B21;">소송</span>
          <span style="font-family:var(--mono);font-size:11px;">${s.no||'-'}</span>
          <button class="btn sm" style="margin-left:auto;font-size:11px;" data-goto-suit="${s.id}">바로가기 \u2192</button>
        </div>
        <div style="font-size:12px;color:var(--tx2);">다음기일: ${s.nextDate||'-'} / 결과: ${s.result||'미확정'}</div>
      </div>`));
      if(!parts.length)return '';
      return `<div class="card" style="margin-bottom:13px;"><h3 style="margin-bottom:10px;">연결된 건</h3>${parts.join('')}</div>`;
    })()}`;
...
function openSuitDetail(id){
  ...
  $('suit-detail-content').innerHTML=`
    ...
    ${(()=>{
      const lc=s.claimRef?claims.find(x=>x.id===s.claimRef):null;
      ...
      if(lc)parts.push(`<div style="padding:10px 12px;background:var(--bg2);border:0.5px solid var(--bd2);border-radius:var(--r-md);margin-bottom:7px;">
        ...
        <div style="font-size:12px;color:var(--tx2);">${lc.name||'-'} / ${lc.type||'-'} / ${lc.insDate||lc.date||'-'}</div>
      </div>`);
      if(lm)parts.push(`<div style="padding:10px 12px;background:#EAF3DE;border:0.5px solid #C0DD97;border-radius:var(--r-md);margin-bottom:7px;">
        <div style="display:flex;align-items:center;gap:7px;margin-bottom:4px;">
          <span style="font-size:11px;font-weight:600;color:var(--green);">소액클레임</span>
        ...
        <div style="font-size:12px;color:var(--tx2);">${lm.name||'-'} / ${lm.type||'-'}</div>
      </div>`);
      if(lca)parts.push(`<div style="padding:10px 12px;background:#E6F1FB;border:0.5px solid #B5D4F4;border-radius:var(--r-md);margin-bottom:7px;">
        <div style="display:flex;align-items:center;gap:7px;margin-bottom:4px;">
          <span style="font-size:11px;font-weight:600;color:var(--blue);">소비자원 ${lca.type}</span>
          <span style="font-family:var(--mono);font-size:11px;">${lca.no||'-'}</span>
          <span class="bdg b-접수" style="font-size:10px;">${lca.status}</span>
          <button class="btn sm" style="margin-left:auto;font-size:11px;" data-goto-ca="${lca.id}">바로가기 \u2192</button>
        </div>
      </div>`);
      if(!parts.length)return '';
      return `<div class="card" style="margin-bottom:13px;"><h3 style="margin-bottom:10px;">연결된 건</h3>${parts.join('')}</div>`;
