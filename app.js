...
function renderDash(){
  ...
  // 소비자원 승소율 계산
  const caTotal=consumerCases.length;
  const caWon=consumerCases.filter(c=>c.result&&(c.result.includes('성립')||c.result.includes('합의'))).length;
  const caRatio=caTotal>0?Math.round(caWon/caTotal*100):null;
  const totalClaimAmt=consumerCases.reduce((a,c)=>a+(c.claimAmount||0),0);
  const totalResultAmt=consumerCases.reduce((a,c)=>a+(c.amount||0),0);
  ...
  $('loss-grid').innerHTML=`
    ...
    ${caTotal>0?`
    <div class="loss-card ${caRatio!==null&&caRatio>=50?'ratio-good':'ratio-bad'}"><div class="loss-label">소비자원 인용률</div><div class="loss-val">${caRatio!==null?caRatio+'%':'-'}</div><div class="loss-sub">${caTotal}건 중 ${caWon}건 성립</div></div>
    <div class="loss-card paid"><div class="loss-label">소비자원 청구금액</div><div class="loss-val">${fmt만원(totalClaimAmt)}</div><div class="loss-sub">전체 기준</div></div>
    <div class="loss-card est"><div class="loss-label">소비자원 결정금액</div><div class="loss-val">${fmt만원(totalResultAmt)}</div><div class="loss-sub">합의/결정 합계</div></div>`:''}
    `;
...
function toggleCAFields(type){
  ...
  ['ca-f-committee-wrap','ca-f-committee2-wrap','ca-f-result-wrap'].forEach(id=>{
...
function openCADetail(id){
  ...
  $('ca-detail-content').innerHTML=`
    ...
    <div class="dg">
      <div class="card">
        ...
        <div class="dr"><span class="dk">담당자</span><span>${c.assignee||'-'}</span></div>
        <div class="dr"><span class="dk">연결 클레임</span><span style="color:var(--blue);font-family:var(--mono);font-size:12px;">${c.claimRef||'-'}</span></div>
        ...
        <div style="border-top:0.5px solid var(--bd3);margin:9px 0;"></div>
        <h3>금액 현황</h3>
        <div class="fi" style="margin-bottom:6px;"><label style="font-size:12px;">청구금액 (원)</label>
          <input type="number" id="ca-claim-amt" value="${c.claimAmount||''}" placeholder="0" style="padding:5px 8px;font-size:12px;border:0.5px solid var(--bd2);border-radius:var(--r-md);background:var(--bg1);color:var(--tx1);font-family:var(--font);width:100%;">
        </div>
        <div class="dr"><span class="dk">결정/합의 금액</span><span style="font-weight:500;color:var(--green);">${c.amount?fmt만원(c.amount):'-'}</span></div>
        ...
        <div style="border-top:0.5px solid var(--bd3);margin:9px 0;"></div>
        <h3>분쟁조정 정보</h3>
        <div class="dr"><span class="dk">1차 위원회</span><span>${c.committeeDate||'-'}</span></div>
        <div class="dr"><span class="dk">2차 위원회</span><span>${c.committeeDate2||'-'}</span></div>
        <div class="dr"><span class="dk">조정 결과</span><span style="font-weight:500;">${c.result||'-'}</span></div>`:''}
      </div>
      <div class="card">
        <h3>피해 내용</h3>
        <p style="font-size:13px;line-height:1.7;">${c.desc||'-'}</p>
        ...
        <div style="margin-top:9px;display:flex;gap:5px;align-items:center;flex-wrap:wrap;">
          ...
          <select id="ca-st-sel" style="padding:4px 7px;font-size:12px;border:0.5px solid var(--bd2);border-radius:var(--r-md);background:var(--bg1);color:var(--tx1);font-family:var(--font);">
            ${['접수','조사','합의','결정','종결','취하'].map(s=>`<option ${c.status===s?'selected':''}>${s}</option>`).join('')}
          </select>
  ...
  // 청구금액 저장
  const camt=$('ca-claim-amt');
  if(camt)camt.addEventListener('change',()=>{
    c.claimAmount=Number(camt.value)||0;persist();openCADetail(c.id);
  });
}
