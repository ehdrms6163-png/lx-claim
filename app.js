...
function nav(name,btn){
  ...
  if(name==='search'){renderGlobalSearch();}
  if(name==='minor'){renderMinorDash();renderMinor();}
  if(name==='minor-detail'){/* openMinorDetail에서 직접 처리 */}
  if(name==='consumer-detail'){/* openCADetail에서 직접 처리 */}
  if(name==='consumer-edit'){/* openCAEditPage에서 직접 처리 */}
  if(name==='suit-detail'){/* openSuitDetail에서 직접 처리 */}
  if(name==='suit-edit'){/* openSuitEditPage에서 직접 처리 */}
  if(name==='consumer'){renderCAStats();renderCA();}
  if(name==='lawsuit'){renderSuit();}
  if(name==='reports'){renderReports();}
  if(name==='settings'){renderAllSettings();renderPolicyList();}
}
...
function populateListFilters(){
  ...
  if(lg){
    const cur=lg.value;
    const lgs=[...new Set(claims.map(c=>c.logistics).filter(Boolean))].sort();
    lg.innerHTML='<option value="">전체 물류</option>'+lgs.map(l=>`<option value="${l}" ${l===cur?'selected':''}>${l}</option>`).join('');
  }
}
function renderList(){
  const q=($('s-q')||{}).value||'',st=($('s-st')||{}).value||'',ty=($('s-ty')||{}).value||'',cs=($('s-cs')||{}).value||'',slg=($('s-lg')||{}).value||'';
  const f=claims.filter(c=>{
    const m=!q||(c.name+c.id+c.desc+(c.tname||'')+(c.client||'')+(c.orderNo||'')+(c.insNo||'')+(c.addr||'')+(c.logistics||'')).toLowerCase().includes(q.toLowerCase());
    return m&&(!st||c.status===st)&&(!ty||c.type===ty)&&(!cs||c.client===cs)&&(!slg||c.logistics===slg);
  });
...
function saveClaim(){
  ...
  persist();
  // 소액클레임 연결 처리 - 비고에 소액클레임 ID 있으면 claimRef 업데이트
  const noteVal=d.note||'';
  const mnMatch=noteVal.match(/소액클레임\s+([\w-]+)\s+연결/);
  if(mnMatch){
    const mnId=mnMatch[1];
    const mn=minorClaims.find(x=>x.id===mnId);
    if(mn){
      mn.claimRef=newId;
      if(!mn.history)mn.history=[];
      mn.history.push({date:today,text:`보험클레임 등록 완료: ${newId}`});
      persist();
    }
  }
...
function _initHandlers(){
  ...
  document.addEventListener('click',e=>{
    ...
    while(el&&el!==document.body){
      if(el.dataset){
        const d=el.dataset;
        if(d.gotoClaim){openDetail(d.gotoClaim);return;}
        if(d.gotoMinor){openMinorDetail(d.gotoMinor);return;}
        if(d.gotoCa){openCADetail(d.gotoCa);return;}
        if(d.gotoSuit){openSuitDetail(d.gotoSuit);return;}
        if(d.mailAccident){sendAccidentReportMail(d.mailAccident);return;}
        if(d.mailInsurance){sendInsuranceMail(d.mailInsurance);return;}
        if(d.epPrefill){try{prefillFromInsRow(JSON.parse(decodeURIComponent(d.epPrefill)));}catch(e){console.error(e);}return;}
        ...
        if(d.openSuit){openSuitDetail(d.openSuit);return;}
        if(d.insColor){
        ...
        // 소액클레임 행 클릭
        if(d.minorId){openMinorDetail(d.minorId);return;}
      }
      el=el.parentElement;
    }
  });
  ...
  // 통합조회 버튼
  const gsBtn=$('btn-gs-search');if(gsBtn)gsBtn.addEventListener('click',renderGlobalSearch);
  const gsQ=$('gs-q');if(gsQ)gsQ.addEventListener('keydown',e=>{if(e.key==='Enter')renderGlobalSearch();});
  // 소액클레임 대시보드 필터
  ['mn-d-from','mn-d-to','mn-d-lg','mn-d-type'].forEach(id=>{
    const el=$(id);if(el)el.addEventListener('change',()=>{renderMinorDash();});
  });
  const mndMonth=$('mn-d-month');
  if(mndMonth)mndMonth.addEventListener('click',()=>{
    const now=new Date();
    const y=now.getFullYear(),m=String(now.getMonth()+1).padStart(2,'0');
    const from=$('mn-d-from');const to=$('mn-d-to');
    if(from)from.value=`${y}-${m}-01`;
    if(to)to.value=`${y}-${m}-${new Date(y,now.getMonth()+1,0).getDate()}`;
    renderMinorDash();
  });
  const mndAll=$('mn-d-all');
  if(mndAll)mndAll.addEventListener('click',()=>{
    const from=$('mn-d-from');const to=$('mn-d-to');
  ...
  // 소비자원 상세 버튼
  // 소비자원 상세 버튼
  const caback=$('btn-ca-back');if(caback)caback.addEventListener('click',()=>nav('consumer',document.querySelector('.nb[data-fn="navConsumer"]')));
  const caedit=$('btn-ca-edit');if(caedit)caedit.addEventListener('click',()=>{if(curCADetail)openCAEditPage(curCADetail.id);});
  const cadel=$('btn-ca-delete');if(cadel)cadel.addEventListener('click',()=>{if(curCADetail)deleteCA(curCADetail.id);});
  // 소비자원 수정 페이지 버튼
  const caeback=$('btn-ca-edit-back');if(caeback)caeback.addEventListener('click',()=>{if(curCADetail)openCADetail(curCADetail.id);else nav('consumer',document.querySelector('.nb[data-fn="navConsumer"]'));});
  const caesave=$('btn-ca-edit-save');if(caesave)caesave.addEventListener('click',saveCAEdit);
  // 소송 상세 버튼
  const suitback=$('btn-suit-back');if(suitback)suitback.addEventListener('click',()=>nav('lawsuit',document.querySelector('.nb[data-fn="navLawsuit"]')));
  const suitedit=$('btn-suit-detail-edit');if(suitedit)suitedit.addEventListener('click',()=>{if(curSuitDetail)openSuitEditPage();});
  const suitdel=$('btn-suit-detail-delete');if(suitdel)suitdel.addEventListener('click',()=>{if(curSuitDetail)deleteSuit(curSuitDetail.id);});
  // 소송 수정 페이지 버튼
  const suiteback=$('btn-suit-edit-back');if(suiteback)suiteback.addEventListener('click',()=>{if(curSuitDetail)openSuitDetail(curSuitDetail.id);});
  const suitesave=$('btn-suit-edit-save');if(suitesave)suitesave.addEventListener('click',saveSuitEdit);
  // 소액클레임 상세 버튼
  const mnback=$('btn-minor-back');if(mnback)mnback.addEventListener('click',()=>nav('minor',document.querySelector('.nb[data-fn="navMinor"]')));
  const mnedit=$('btn-minor-edit');if(mnedit)mnedit.addEventListener('click',()=>{if(curMinorDetail)openMinorForm(curMinorDetail.id);});
  const mndel=$('btn-minor-delete');if(mndel)mndel.addEventListener('click',()=>{if(curMinorDetail)deleteMinor(curMinorDetail.id);});
...
/* ══════════════════════════════════════════════
   소액클레임
══════════════════════════════════════════════ */
let curMinorId=null, curMinorDetail=null;
...
function renderMinorDash(){
  const from=($('mn-d-from')||{}).value||'';
  const to=($('mn-d-to')||{}).value||'';
  const flg=($('mn-d-lg')||{}).value||'';
  const fty=($('mn-d-type')||{}).value||'';

  // 물류 드롭다운 채우기
  const dlg=$('mn-d-lg');
  if(dlg){const cur=dlg.value;const lgs=[...new Set(minorClaims.map(c=>c.logistics).filter(Boolean))].sort();dlg.innerHTML='<option value="">전체</option>'+lgs.map(l=>`<option value="${l}" ${l===cur?'selected':''}>${l}</option>`).join('');}

  const filtered=minorClaims.filter(c=>{
    const dt=c.adate||c.date||'';
    if(from&&dt<from)return false;
    if(to&&dt>to)return false;
    if(flg&&c.logistics!==flg)return false;
    if(fty&&c.type!==fty)return false;
  ...
  const tot=filtered.length;
  const inProgress=filtered.filter(c=>!['종결','보험접수'].includes(c.status)).length;
  const ended=filtered.filter(c=>c.status==='종결').length;
  const linked=filtered.filter(c=>c.status==='보험접수'||c.claimRef).length;
  const caLinked=filtered.filter(c=>c.status==='소비자원접수').length;

  const sg=$('mn-stat-grid');
  if(sg)sg.innerHTML=`
    <div class="sc"><div class="sl">전체 건수</div><div class="sv bl">${tot}건</div></div>
    <div class="sc"><div class="sl">진행 중</div><div class="sv am">${inProgress}건</div></div>
    <div class="sc"><div class="sl">보험접수 연결</div><div class="sv gr">${linked}건</div></div>
    <div class="sc"><div class="sl">종결</div><div class="sv gr">${ended}건</div></div>`;

  const tc=['#378ADD','#EF9F27','#7F77DD','#639922','#E24B4A','#028090','#C0392B'];

  // 물류별 차트
  const byL={};filtered.forEach(c=>{if(c.logistics){byL[c.logistics]=(byL[c.logistics]||0)+1;}});
  const topL=Object.entries(byL).sort((a,b)=>b[1]-a[1]).slice(0,8);
  const lmax=Math.max(...topL.map(x=>x[1]),1);
  const lc=$('mn-logistics-chart');
  if(lc)lc.innerHTML=topL.length?topL.map(([name,cnt],i)=>`<div class="bar-r"><span class="bar-lbl" style="font-size:11px;">${name}</span><div class="bar-track"><div class="bar-fill" style="width:${Math.round(cnt/lmax*100)}%;background:${tc[i%7]};"></div></div><span class="bar-val">${cnt}</span></div>`).join(''):'<div style="font-size:12px;color:var(--tx3);padding:8px 0;">데이터 없음</div>';

  // 유형별 차트
  const byT={};filtered.forEach(c=>{if(c.type){byT[c.type]=(byT[c.type]||0)+1;}});
  const topT=Object.entries(byT).sort((a,b)=>b[1]-a[1]);
  const tmax=Math.max(...topT.map(x=>x[1]),1);
  const tc2=$('mn-type-chart');
  if(tc2)tc2.innerHTML=topT.length?topT.map(([name,cnt],i)=>`<div class="bar-r"><span class="bar-lbl" style="font-size:11px;">${name}</span><div class="bar-track"><div class="bar-fill" style="width:${Math.round(cnt/tmax*100)}%;background:${tc[i%7]};"></div></div><span class="bar-val">${cnt}</span></div>`).join(''):'<div style="font-size:12px;color:var(--tx3);padding:8px 0;">데이터 없음</div>';
}
...
function renderMinor(){
  ...
  const filtered=minorClaims.filter(c=>{
    const m=!q||(c.name+(c.addr||'')+(c.order||'')+(c.logistics||'')+(c.partner||'')+(c.desc||'')+(c.id||'')).toLowerCase().includes(q.toLowerCase());
    return m&&(!st||c.status===st)&&(!ty||c.type===ty);
  });
  const el=$('minor-list');if(!el)return;
  el.innerHTML=filtered.length?filtered.map(c=>`
    <div class="tr" style="grid-template-columns:110px 80px 1fr 80px 60px 70px 80px 80px;cursor:pointer;" data-minor-id="${c.id}">
      <span style="font-family:var(--mono);font-size:10px;color:var(--blue);">${c.id}</span>
...
function openMinorDetail(id){
  curMinorDetail=minorClaims.find(x=>x.id===id);
  if(!curMinorDetail)return;
  const m=curMinorDetail;
  document.querySelectorAll('.sec').forEach(s=>s.classList.remove('active'));
  document.querySelectorAll('.nb').forEach(b=>b.classList.remove('active'));
  $('s-minor-detail').classList.add('active');
  const minorBtn=document.querySelector('.nb[data-fn="navMinor"]');
  if(minorBtn)minorBtn.classList.add('active');
  $('minor-detail-title').textContent=m.id;
  const steps=['접수','조사중','합의중','보험접수','종결'];
  const si=steps.indexOf(m.status);
  $('minor-detail-content').innerHTML=`
    <div class="ps">${steps.map((s,i)=>`<div class="pst ${i<si?'done':i===si?'active':''}"><div class="pc">${i<si?'<svg class="ico ico-md"><use href="#ico-check"/></svg>':i+1}</div><div class="pl">${s}</div></div>`).join('')}</div>
    <div class="dg">
      <div class="card">
        <h3>기본 정보</h3>
        <div class="dr"><span class="dk">고객명</span><span style="font-weight:500;">${m.name}</span></div>
        <div class="dr"><span class="dk">연락처</span><span>${m.phone||'-'}</span></div>
        <div class="dr"><span class="dk">주소</span><span style="font-size:12px;">${m.addr||'-'}</span></div>
        <div class="dr"><span class="dk">주문번호</span><span style="font-family:var(--mono);font-size:12px;">${m.order||'-'}</span></div>
        <div class="dr"><span class="dk">물류센터</span><span>${m.logistics||'-'}</span></div>
        <div class="dr"><span class="dk">협력업체</span><span>${m.partner||'-'}</span></div>
        <div class="dr"><span class="dk">설치기사</span><span>${m.tech||'-'}</span></div>
        <div class="dr"><span class="dk">제품구분</span><span>${m.pcat||'-'}</span></div>
        <div class="dr"><span class="dk">사고유형</span><span>${m.type||'-'}</span></div>
        <div class="dr"><span class="dk">설치일자</span><span>${m.idate||'-'}</span></div>
        <div class="dr"><span class="dk">사고발생일</span><span>${m.adate||'-'}</span></div>
        <div class="dr"><span class="dk">피해금액 추산</span><span>${m.amount?m.amount.toLocaleString('ko-KR')+'원':'-'}</span></div>
        ${m.claimRef?`<div class="dr"><span class="dk">보험접수 연결</span><span style="color:var(--blue);font-family:var(--mono);font-size:12px;">${m.claimRef}</span></div>`:''}
        ${m.adjuster?`<div class="dr"><span class="dk">손해사정 담당자</span><span>${m.adjuster}</span></div>`:''}
      </div>
      <div class="card">
        <h3>사고 내용</h3>
        <p style="font-size:13px;line-height:1.7;">${m.desc||'-'}</p>
        ${m.note?`<div style="margin-top:7px;padding:7px 9px;background:var(--bg2);border-radius:var(--r-md);font-size:12px;color:var(--tx2);"><b style="font-weight:500;">비고:</b> ${m.note}</div>`:''}
        <div style="border-top:0.5px solid var(--bd3);margin:12px 0;"></div>
        <h3>처리 이력</h3>
        <div id="minor-history-list">${renderHistoryEditable(m.history||[],()=>{persist();openMinorDetail(m.id);})}</div>
        <div style="display:flex;gap:7px;margin-top:9px;">
          <input type="text" id="minor-hist-inp" placeholder="처리 내용..." style="flex:1;padding:5px 8px;font-size:12px;border:0.5px solid var(--bd2);border-radius:var(--r-md);background:var(--bg1);color:var(--tx1);font-family:var(--font);">
          <input type="date" id="minor-hist-date" style="padding:5px 8px;font-size:12px;border:0.5px solid var(--bd2);border-radius:var(--r-md);background:var(--bg1);color:var(--tx1);font-family:var(--font);">
          <button class="btn sm" id="btn-minor-add-hist">추가</button>
        </div>
        <div style="margin-top:9px;display:flex;gap:5px;align-items:center;">
          <span style="font-size:12px;color:var(--tx2);">상태:</span>
          <select id="minor-st-sel" style="padding:4px 7px;font-size:12px;border:0.5px solid var(--bd2);border-radius:var(--r-md);background:var(--bg1);color:var(--tx1);font-family:var(--font);">
            ${steps.map(s=>`<option ${m.status===s?'selected':''}>${s}</option>`).join('')}
          </select>
          <button class="btn sm pri" id="btn-minor-chg-status">적용</button>
        </div>
        ${!m.claimRef?`
        <div style="margin-top:12px;padding:11px;background:#EAF3DE;border:0.5px solid #C0DD97;border-radius:var(--r-md);">
          <div style="font-size:12px;font-weight:500;color:var(--green);margin-bottom:6px;">보험접수 연결</div>
          <div style="font-size:12px;color:var(--tx2);margin-bottom:8px;">이 소액클레임을 보험접수 클레임으로 연결합니다. 처리이력이 함께 이어집니다.</div>
    ...
    ${renderLinkedCard({claimId:m.claimRef||null})}`;

  // 이벤트 바인딩
  bindHistoryEditable(m.history||[],'#minor-history-list',()=>{persist();openMinorDetail(m.id);});
  const bah=$('btn-minor-add-hist');
  if(bah)bah.addEventListener('click',()=>{
    const inp=$('minor-hist-inp');if(!inp||!inp.value.trim())return;
    const dt=($('minor-hist-date')||{}).value||new Date().toISOString().slice(0,10);
    if(!m.history)m.history=[];
    m.history.push({date:dt,text:inp.value.trim()});
    inp.value='';
    persist();openMinorDetail(m.id);
  });
  const bcs=$('btn-minor-chg-status');
  if(bcs)bcs.addEventListener('click',()=>{
    const sel=$('minor-st-sel');if(!sel)return;
    const old=m.status;m.status=sel.value;
    if(old!==sel.value){
      if(!m.history)m.history=[];
      m.history.push({date:new Date().toISOString().slice(0,10),text:'상태 변경: '+old+' > '+sel.value});
    }
    persist();openMinorDetail(m.id);
  });
  const blc=$('btn-minor-link-claim');
  if(blc)blc.addEventListener('click',()=>linkMinorToClaim(m.id));
  const blca=$('btn-minor-link-ca');
  if(blca)blca.addEventListener('click',()=>linkMinorToCA(m.id));
}
...
function openMinorForm(id){
  ...
  const mlt=$('mn-f-lineteam');if(mlt)mlt.value=c.lineteam||'';
...
function saveMinor(){
  ...
  if(!name)return;
  const today=new Date().toISOString().slice(0,10);
  const existing=curMinorId?minorClaims.find(x=>x.id===curMinorId):null;
  const d={
    id:curMinorId||genMinorId(),
    name,phone:$('mn-f-phone').value,addr:$('mn-f-addr').value,
    order:$('mn-f-order').value,logistics:$('mn-f-logistics').value,lineteam:($('mn-f-lineteam')||{}).value||'',
    ...
    history:existing?.history||[{date:today,text:'소액클레임 접수'}],
  };
  if(curMinorId){const i=minorClaims.findIndex(x=>x.id===curMinorId);if(i>=0)minorClaims[i]=d;}
  else minorClaims.unshift(d);
  persist();$('minor-modal').classList.remove('open');
  renderMinor();
}

function linkMinorToCA(minorId){
  ...
  document.querySelectorAll('.sec').forEach(s=>s.classList.remove('active'));
  document.querySelectorAll('.nb').forEach(b=>b.classList.remove('active'));
  $('s-consumer').classList.add('active');
  const caBtn=document.querySelector('.nb[data-fn="navConsumer"]');
...
function deleteMinor(id){
  minorClaims=minorClaims.filter(x=>x.id!==id);
  persist();
  nav('minor',document.querySelector('.nb[data-fn="navMinor"]'));
}
...
function linkMinorToClaim(minorId){
  ...
  setTimeout(()=>{
    ...
    m.status='보험접수';
    if(!m.history)m.history=[];
    m.history.push({date:new Date().toISOString().slice(0,10),text:'보험접수 연결 - 클레임 등록 진행'});
    persist();
    const banner=$('autofill-banner');
...
/* ══════════════════════════════════════════════
   처리이력 수정/삭제 공통
══════════════════════════════════════════════ */
function renderHistoryEditable(history, onUpdate){
  if(!history||!history.length)return '<div style="font-size:12px;color:var(--tx3);padding:4px 0;">이력 없음</div>';
  let html='';
  history.forEach((h,i)=>{
    if(h.stage&&!h.text){
      // stage만 있는 순수 구분선
      html+=`<div style="display:flex;align-items:center;gap:6px;margin:10px 0 6px;">
        ...
        <span style="font-size:11px;font-weight:600;color:var(--blue);white-space:nowrap;padding:0 6px;">${h.stage}</span>
        <button class="btn sm icon dng" data-hist-del="${i}" title="삭제" style="flex-shrink:0;"><svg class="ico ico-sm"><use href="#ico-trash"/></svg></button>
        <div style="height:1px;flex:1;background:var(--bd3);"></div>
      </div>`;
    } else {
...
function bindHistoryEditable(history, containerSelector, onUpdate){
  const container=document.querySelector(containerSelector);
  if(!container)return;
  container.querySelectorAll('[data-hist-edit]').forEach(btn=>{
    btn.addEventListener('click',()=>{
      const i=Number(btn.dataset.histEdit);
      const h=history[i];if(!h)return;
      // 인라인 편집 UI 생성
      const tl=btn.closest('.tl-i');if(!tl)return;
      const textDiv=tl.querySelector(`#hist-text-${i}`);
      const dtDiv=tl.querySelector('.tl-dt');
      if(tl.querySelector('.hist-edit-form'))return; // 이미 열려있으면 무시
      const form=document.createElement('div');
      form.className='hist-edit-form';
      form.style.cssText='margin-top:6px;display:flex;flex-direction:column;gap:4px;';
      form.innerHTML=`
        <input type="text" class="he-text" value="${(h.text||'').replace(/"/g,'&quot;')}" style="padding:4px 7px;font-size:12px;border:0.5px solid var(--bd2);border-radius:var(--r-md);background:var(--bg1);color:var(--tx1);font-family:var(--font);">
        <div style="display:flex;gap:4px;">
          <input type="date" class="he-date" value="${h.date||''}" style="padding:4px 7px;font-size:12px;border:0.5px solid var(--bd2);border-radius:var(--r-md);background:var(--bg1);color:var(--tx1);font-family:var(--font);">
          <button class="btn sm pri he-save">저장</button>
          <button class="btn sm he-cancel">취소</button>
        </div>`;
      tl.appendChild(form);
      form.querySelector('.he-save').addEventListener('click',()=>{
        h.text=form.querySelector('.he-text').value.trim();
        h.date=form.querySelector('.he-date').value||h.date;
        onUpdate();
      });
      form.querySelector('.he-cancel').addEventListener('click',()=>form.remove());
    });
  });
  container.querySelectorAll('[data-hist-del]').forEach(btn=>{
    btn.addEventListener('click',()=>{
      const i=Number(btn.dataset.histDel);
      history.splice(i,1);
      onUpdate();
    });
  });
...
function buildLinkedHtml(items){
  ...
  return '<div class="card" style="margin-bottom:13px;"><h3 style="margin-bottom:10px;">연결된 건</h3>'+items.join('')+'</div>';
}
function buildSuitLinkedHtml(claimRef){
  const lc=claimRef?claims.find(x=>x.id===claimRef):null;
  const lm=lc?minorClaims.find(x=>x.claimRef===lc.id):null;
  const lca=lc?consumerCases.find(x=>x.claimRef===lc.id||(lm&&x.minorRef===lm.id)):null;
  const parts=[];
  if(lc)parts.push('<div style="padding:10px 12px;background:var(--bg2);border:0.5px solid var(--bd2);border-radius:var(--r-md);margin-bottom:7px;"><div style="display:flex;align-items:center;gap:7px;margin-bottom:4px;"><span style="font-size:11px;font-weight:600;color:var(--tx2);">보험 클레임</span><span style="font-family:var(--mono);font-size:11px;color:var(--blue);">'+lc.id+'</span><span class="bdg b-'+lc.status+'" style="font-size:10px;">'+lc.status+'</span><button class="btn sm" style="margin-left:auto;font-size:11px;" data-goto-claim="'+lc.id+'">바로가기</button></div><div style="font-size:12px;color:var(--tx2);">'+(lc.name||'-')+' / '+(lc.type||'-')+'</div></div>');
  if(lm)parts.push('<div style="padding:10px 12px;background:#EAF3DE;border:0.5px solid #C0DD97;border-radius:var(--r-md);margin-bottom:7px;"><div style="display:flex;align-items:center;gap:7px;margin-bottom:4px;"><span style="font-size:11px;font-weight:600;color:var(--green);">소액클레임</span><span style="font-family:var(--mono);font-size:11px;color:var(--green);">'+lm.id+'</span><span class="bdg b-접수" style="font-size:10px;">'+lm.status+'</span><button class="btn sm" style="margin-left:auto;font-size:11px;" data-goto-minor="'+lm.id+'">바로가기</button></div><div style="font-size:12px;color:var(--tx2);">'+(lm.name||'-')+' / '+(lm.type||'-')+'</div></div>');
  if(lca)parts.push('<div style="padding:10px 12px;background:#E6F1FB;border:0.5px solid #B5D4F4;border-radius:var(--r-md);margin-bottom:7px;"><div style="display:flex;align-items:center;gap:7px;margin-bottom:4px;"><span style="font-size:11px;font-weight:600;color:var(--blue);">소비자원 '+lca.type+'</span><span style="font-family:var(--mono);font-size:11px;">'+(lca.no||'-')+'</span><button class="btn sm" style="margin-left:auto;font-size:11px;" data-goto-ca="'+lca.id+'">바로가기</button></div></div>');
  return buildLinkedHtml(parts);
}
function buildCALinkedHtml(minorRef, claimRef){
  const lm=minorRef?minorClaims.find(x=>x.id===minorRef):null;
  const lc=claimRef?claims.find(x=>x.id===claimRef):(lm&&lm.claimRef?claims.find(x=>x.id===lm.claimRef):null);
  const ls=lc?lawsuits.filter(x=>x.claimRef===lc.id):[];
  const parts=[];
  if(lm)parts.push('<div style="padding:10px 12px;background:#EAF3DE;border:0.5px solid #C0DD97;border-radius:var(--r-md);margin-bottom:7px;"><div style="display:flex;align-items:center;gap:7px;margin-bottom:4px;"><span style="font-size:11px;font-weight:600;color:var(--green);">소액클레임</span><span style="font-family:var(--mono);font-size:11px;color:var(--green);">'+lm.id+'</span><span class="bdg b-접수" style="font-size:10px;">'+lm.status+'</span><button class="btn sm" style="margin-left:auto;font-size:11px;" data-goto-minor="'+lm.id+'">바로가기</button></div><div style="font-size:12px;color:var(--tx2);">'+(lm.name||'-')+' / '+(lm.type||'-')+'</div></div>');
  if(lc)parts.push('<div style="padding:10px 12px;background:var(--bg2);border:0.5px solid var(--bd2);border-radius:var(--r-md);margin-bottom:7px;"><div style="display:flex;align-items:center;gap:7px;margin-bottom:4px;"><span style="font-size:11px;font-weight:600;color:var(--tx2);">보험 클레임</span><span style="font-family:var(--mono);font-size:11px;color:var(--blue);">'+lc.id+'</span><span class="bdg b-'+lc.status+'" style="font-size:10px;">'+lc.status+'</span><button class="btn sm" style="margin-left:auto;font-size:11px;" data-goto-claim="'+lc.id+'">바로가기</button></div><div style="font-size:12px;color:var(--tx2);">'+(lc.name||'-')+' / '+(lc.type||'-')+'</div></div>');
  ls.forEach(s=>parts.push('<div style="padding:10px 12px;background:#FCEBEB;border:0.5px solid #F7C1C1;border-radius:var(--r-md);margin-bottom:7px;"><div style="display:flex;align-items:center;gap:7px;margin-bottom:4px;"><span style="font-size:11px;font-weight:600;color:#922B21;">소송</span><span style="font-family:var(--mono);font-size:11px;">'+(s.no||'-')+'</span><button class="btn sm" style="margin-left:auto;font-size:11px;" data-goto-suit="'+s.id+'">바로가기</button></div><div style="font-size:12px;color:var(--tx2);">다음기일: '+(s.nextDate||'-')+'</div></div>'));
  return buildLinkedHtml(parts);
}
function renderLinkedCard({claimId, minorId, caId, suitId, selfClaimId}={}){
  // 보험클레임
  const linkedClaim=claimId?claims.find(x=>x.id===claimId):null;
  // 소액클레임 상세에서 claimRef가 없으면 minorId로 소비자원/소송 탐색
  // 소액클레임 - claimRef로만 찾기 (자기 자신 제외)
  const linkedMinor=claimId?minorClaims.find(x=>x.claimRef===claimId):null;
  // 소비자원 - claimRef 또는 minorId로 찾기 (caId는 자기 자신 제외)
  const linkedCA=caId?null:consumerCases.find(x=>{
    if(claimId&&x.claimRef===claimId)return true;
    if(minorId&&x.minorRef===minorId)return true;
    if(linkedMinor&&x.minorRef===linkedMinor.id)return true;
    return false;
  })||null;
  // 소송 - claimRef로 찾기 (suitId는 자기 자신 제외)
  const linkedSuits=suitId?[]:claimId?lawsuits.filter(x=>x.claimRef===claimId):[];

  const parts=[];

  // selfClaimId면 자기 자신 보험클레임 카드 제외
  if(linkedClaim&&linkedClaim.id!==selfClaimId)parts.push(`
    <div style="padding:10px 12px;background:var(--bg2);border:0.5px solid var(--bd2);border-radius:var(--r-md);margin-bottom:7px;">
      <div style="display:flex;align-items:center;gap:7px;margin-bottom:4px;">
        <span style="font-size:11px;font-weight:600;color:var(--tx2);">보험 클레임</span>
        <span style="font-family:var(--mono);font-size:11px;color:var(--blue);">${linkedClaim.id}</span>
        <span class="bdg b-${linkedClaim.status}" style="font-size:10px;">${linkedClaim.status}</span>
        <button class="btn sm" style="margin-left:auto;font-size:11px;" data-goto-claim="${linkedClaim.id}">바로가기 \u2192</button>
      </div>
      <div style="font-size:12px;color:var(--tx2);">${linkedClaim.name||'-'} / ${linkedClaim.type||'-'} / ${linkedClaim.insDate||linkedClaim.date||'-'}</div>
      <div style="font-size:12px;color:var(--tx2);">${linkedClaim.addr||''} ${linkedClaim.tname?'· 기사: '+linkedClaim.tname:''}</div>
    </div>`);

  if(linkedMinor)parts.push(`
    <div style="padding:10px 12px;background:#EAF3DE;border:0.5px solid #C0DD97;border-radius:var(--r-md);margin-bottom:7px;">
      <div style="display:flex;align-items:center;gap:7px;margin-bottom:4px;">
        <span style="font-size:11px;font-weight:600;color:var(--green);">소액클레임</span>
        <span style="font-family:var(--mono);font-size:11px;color:var(--green);">${linkedMinor.id}</span>
        <span class="bdg b-접수" style="font-size:10px;">${linkedMinor.status}</span>
        <button class="btn sm" style="margin-left:auto;font-size:11px;" data-goto-minor="${linkedMinor.id}">바로가기 \u2192</button>
      </div>
      <div style="font-size:12px;color:var(--tx2);">${linkedMinor.name||'-'} / ${linkedMinor.type||'-'} / ${linkedMinor.adate||linkedMinor.idate||'-'}</div>
      <div style="font-size:12px;color:var(--tx2);">${linkedMinor.logistics||''} ${linkedMinor.tech?'· 기사: '+linkedMinor.tech:''} ${linkedMinor.order?'· 주문: '+linkedMinor.order:''}</div>
    </div>`);

  if(linkedCA)parts.push(`
    <div style="padding:10px 12px;background:#E6F1FB;border:0.5px solid #B5D4F4;border-radius:var(--r-md);margin-bottom:7px;">
      <div style="display:flex;align-items:center;gap:7px;margin-bottom:4px;">
        <span style="font-size:11px;font-weight:600;color:var(--blue);">소비자원 ${linkedCA.type}</span>
        <span style="font-family:var(--mono);font-size:11px;">${linkedCA.no||'-'}</span>
        <span class="bdg b-접수" style="font-size:10px;">${linkedCA.status}</span>
        <button class="btn sm" style="margin-left:auto;font-size:11px;" data-goto-ca="${linkedCA.id}">바로가기 \u2192</button>
      </div>
      <div style="font-size:12px;color:var(--tx2);">${linkedCA.claimant||'-'} / 청구: ${linkedCA.claimAmount?linkedCA.claimAmount.toLocaleString()+'원':'-'} / 결정: ${linkedCA.amount?linkedCA.amount.toLocaleString()+'원':'-'}</div>
    </div>`);

  linkedSuits.forEach(s=>parts.push(`
    <div style="padding:10px 12px;background:#FCEBEB;border:0.5px solid #F7C1C1;border-radius:var(--r-md);margin-bottom:7px;">
      <div style="display:flex;align-items:center;gap:7px;margin-bottom:4px;">
        <span style="font-size:11px;font-weight:600;color:#922B21;">소송</span>
        <span style="font-family:var(--mono);font-size:11px;">${s.no||'-'}</span>
        <span style="font-size:11px;color:var(--tx2);">${s.court||'-'} ${s.instance||''}</span>
        <button class="btn sm" style="margin-left:auto;font-size:11px;" data-goto-suit="${s.id}">바로가기 \u2192</button>
      </div>
      <div style="font-size:12px;color:var(--tx2);">다음기일: ${s.nextDate||'-'} / 결과: ${s.result||'미확정'} ${s.resultAmount?'/ '+s.resultAmount.toLocaleString()+'원':''}</div>
    </div>`));

  if(!parts.length)return '';
  return `<div class="card" style="margin-bottom:13px;">
    <h3 style="margin-bottom:10px;">연결된 건</h3>
    ${parts.join('')}
  </div>`;
}
...
function renderPolicyList(){
  ...
  el.innerHTML=policySettings.map(p=>{
    ...
    // 현재 유효한 증권인지
    const today=new Date().toISOString().slice(0,10);
    const active=(!p.from||p.from<=today)&&(!p.to||p.to>=today);
    return `<div style="display:flex;align-items:center;gap:8px;padding:8px 10px;background:${active?'var(--bg1)':'var(--bg2)'};border:0.5px solid ${active?'var(--bd2)':'var(--bd3)'};border-radius:var(--r-md);margin-bottom:6px;">
      <div style="flex:1;">
        <div style="display:flex;align-items:center;gap:6px;margin-bottom:3px;">
          <span style="font-size:12px;font-weight:500;">${ins?.name||'-'}</span>
          <span style="font-size:11px;padding:1px 6px;background:${active?'#EAF3DE':'var(--bg2)'};color:${active?'var(--green)':'var(--tx3)'};border-radius:99px;">${active?'유효':'만료'}</span>
          <span style="font-size:11px;color:var(--tx2);">${typeLabel}</span>
        </div>
        <div style="font-size:11px;color:var(--tx2);">증권번호: ${p.no||'-'} · 한도: ${p.limit?p.limit.toLocaleString()+'원':'-'} · ${p.from||'-'} ~ ${p.to||'-'}</div>
      </div>
      <button class="btn sm icon" data-policy-edit="${p.id}"><svg class="ico ico-sm"><use href="#ico-pencil"/></svg></button>
      <button class="btn sm icon dng" data-policy-del="${p.id}"><svg class="ico ico-sm"><use href="#ico-trash"/></svg></button>
    </div>`;
  }).join('');
  // 이벤트 바인딩
  el.querySelectorAll('[data-policy-edit]').forEach(btn=>{
    btn.addEventListener('click',()=>openPolicyModal(btn.dataset.policyEdit));
  });
  el.querySelectorAll('[data-policy-del]').forEach(btn=>{
    btn.addEventListener('click',()=>{
      policySettings=policySettings.filter(x=>x.id!==btn.dataset.policyDel);
      persist();renderPolicyList();renderPolicyStats();
    });
  });
}
function openPolicyModal(id){
  curPolicyId=id||null;
  const p=id?policySettings.find(x=>x.id===id):{};
  $('policy-modal-title').textContent=id?'증권 수정':'증권 등록';
  const ins=$('pm-f-ins');
  if(ins)ins.innerHTML='<option value="">선택</option>'+(insCompanies||[]).map(x=>'<option value="'+x.id+'" '+((p&&p.insId===x.id)?'selected':'')+'>'+x.name+'</option>').join('');
  $('pm-f-no').value=p.no||'';
  $('pm-f-type').value=p.type||'HA';
  $('pm-f-limit').value=p.limit||'';
  $('pm-f-from').value=p.from||'';
  $('pm-f-to').value=p.to||'';
  $('policy-modal').classList.add('open');
}
function savePolicyModal(){
  const d={
    id:curPolicyId||uid('pol'),
    insId:($('pm-f-ins')||{}).value||'',
    no:$('pm-f-no').value||'',
    type:$('pm-f-type').value||'HA',
    limit:Number($('pm-f-limit').value)||0,
    from:$('pm-f-from').value||'',
    to:$('pm-f-to').value||'',
  };
...
function renderPolicyStats(){
  ...
  if(!policySettings.length){el.innerHTML='';return;}
  const today=new Date().toISOString().slice(0,10);
  const activePolicies=policySettings.filter(p=>(!p.from||p.from<=today)&&(!p.to||p.to>=today));
  if(!activePolicies.length){el.innerHTML='';return;}
  let html=`<div style="grid-column:span 4;font-size:12px;font-weight:600;color:var(--tx2);padding-bottom:4px;border-bottom:0.5px solid var(--bd3);">증권별 손해율</div>`;
...
function renderGlobalSearch(){
  const q=($('gs-q')||{}).value||'';
  const flt=($('gs-lineteam')||{}).value||'';
  const el=$('gs-result');if(!el)return;
  if(!q&&!flt){el.innerHTML='<div class="empty">검색어 또는 Line팀을 선택하세요</div>';return;}
  const lq=q.toLowerCase();

  function matchQ(str){return !lq||(str||'').toLowerCase().includes(lq);}
  function matchTeam(lineteam){return !flt||(lineteam||'')===flt;}

  // 전체 검색 (소송/소비자원 포함)
  const matchedClaims=claims.filter(c=>matchTeam(c.lineteam)&&(
    matchQ(c.name)||matchQ(c.tname)||matchQ(c.orderNo)||matchQ(c.product)||
    matchQ(c.addr)||matchQ(c.id)||matchQ(c.insNo)||matchQ(c.desc)||
    matchQ(c.logistics)||matchQ(c.type)||matchQ(c.client)
  ));
  const matchedMinor=minorClaims.filter(m=>matchTeam(m.lineteam)&&(
    matchQ(m.name)||matchQ(m.tech)||matchQ(m.order)||matchQ(m.addr)||
    matchQ(m.id)||matchQ(m.desc)||matchQ(m.logistics)||matchQ(m.type)
  ));
  const matchedCA=consumerCases.filter(c=>
    matchQ(c.claimant)||matchQ(c.no)||matchQ(c.desc)||matchQ(c.type)||matchQ(c.note)
  );
  const matchedSuits=lawsuits.filter(s=>
    matchQ(s.no)||matchQ(s.plaintiff)||matchQ(s.defendant)||matchQ(s.court)||
    matchQ(s.desc)||matchQ(s.type)||matchQ(s.result)
  );
  const matchedEP=epRecs.filter(r=>(
    matchQ(r.고객명)||matchQ(r.설치기사)||matchQ(r.주문번호||r.설치주문번호)||matchQ(r.주소)
  )).slice(0,10);

  if(!matchedClaims.length&&!matchedMinor.length&&!matchedCA.length&&!matchedSuits.length&&!matchedEP.length){
    el.innerHTML='<div class="empty">검색 결과 없음</div>';return;
  }

  function getLinked(claimId,minorId){
    return {
      minor:claimId?minorClaims.find(m=>m.claimRef===claimId)||null:null,
      ca:consumerCases.find(c=>(claimId&&c.claimRef===claimId)||(minorId&&c.minorRef===minorId))||null,
      suit:claimId?lawsuits.find(s=>s.claimRef===claimId)||null:null,
    };
  }
  function linkedBadges(linked,claimRef){
    const parts=[];
    if(claimRef)parts.push(`<span style="font-size:11px;padding:2px 7px;background:#EAF3DE;color:var(--green);border-radius:99px;cursor:pointer;" data-goto-claim="${claimRef}">보험클레임 ${claimRef}</span>`);
    if(linked.minor)parts.push(`<span style="font-size:11px;padding:2px 7px;background:#EAF3DE;color:var(--green);border-radius:99px;cursor:pointer;" data-goto-minor="${linked.minor.id}">소액 ${linked.minor.id}</span>`);
    if(linked.ca)parts.push(`<span style="font-size:11px;padding:2px 7px;background:#E6F1FB;color:var(--blue);border-radius:99px;cursor:pointer;" data-goto-ca="${linked.ca.id}">소비자원 ${linked.ca.type} ${linked.ca.no||''}</span>`);
    if(linked.suit)parts.push(`<span style="font-size:11px;padding:2px 7px;background:#FCEBEB;color:var(--red);border-radius:99px;cursor:pointer;" data-goto-suit="${linked.suit.id}">소송 ${linked.suit.no||''}</span>`);
    return parts.length?`<div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:6px;padding-top:6px;border-top:0.5px solid var(--bd3);">${parts.join('')}</div>`:'';
  }
  ...
  // EP 설치이력
  if(matchedEP.length){
    html+=`<div style="font-size:13px;font-weight:600;color:var(--tx2);margin:0 0 8px;">EP 설치이력 (${matchedEP.length}건)</div>
    <div class="card" style="margin-bottom:14px;padding:0;overflow:hidden;">
      <div class="th" style="grid-template-columns:120px 80px 1fr 90px 70px 70px;"><span>주문번호</span><span>물류</span><span>고객명/주소</span><span>설치기사</span><span>설치일</span><span>신규접수</span></div>
      ${matchedEP.map(r=>`<div class="tr" style="grid-template-columns:120px 80px 1fr 90px 70px 70px;">
        <span style="font-family:var(--mono);font-size:11px;">${r.주문번호||r.설치주문번호||'-'}</span>
        <span style="font-size:11px;">${r.물류센터||r.센터||'-'}</span>
        <span><b style="font-weight:500;">${r.고객명||'-'}</b> <span style="color:var(--tx2);font-size:11px;">${(r.주소||'').slice(0,20)}</span></span>
        <span style="font-size:11px;">${r.설치기사||'-'}</span>
        <span style="font-size:11px;">${r.설치일||'-'}</span>
        <span><button class="btn sm" data-ep-prefill="${encodeURIComponent(JSON.stringify(r))}">접수</button></span>
      </div>`).join('')}
    </div>`;
  }
  ...
  if(matchedClaims.length){
    html+=`<div style="font-size:13px;font-weight:600;color:var(--tx2);margin:0 0 8px;">보험 클레임 (${matchedClaims.length}건)</div>`;
    matchedClaims.forEach(c=>{
      const linked=getLinked(c.id,null);
      const cIns=(insCompanies||[]).find(x=>x.id===c.insCoId);
      html+=`<div class="card" style="margin-bottom:10px;">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:5px;flex-wrap:wrap;">
          <span style="font-family:var(--mono);font-size:11px;color:var(--blue);cursor:pointer;" data-goto-claim="${c.id}">${c.id}</span>
          <span class="bdg b-${c.status}">${c.status}</span>
          <span style="font-size:11px;color:var(--tx2);">${c.type}</span>
          <span style="font-weight:500;">${c.name||'-'}</span>
          <span style="font-size:11px;color:var(--tx2);margin-left:auto;">${c.insDate||c.date||'-'}</span>
        </div>
        <div style="font-size:12px;color:var(--tx2);display:flex;gap:8px;flex-wrap:wrap;">
          ${c.logistics?'<span>📦 '+c.logistics+'</span>':''}
          ${c.lineteam?'<span>👥 '+c.lineteam+'</span>':''}
          ${c.tname?'<span>🔧 '+c.tname+'</span>':''}
          ${c.orderNo?'<span>📋 '+c.orderNo+'</span>':''}
          ${c.addr?'<span>📍 '+c.addr.slice(0,20)+'</span>':''}
        </div>
        ${c.insNo||cIns?`<div style="font-size:12px;color:var(--tx2);margin-top:3px;display:flex;gap:8px;flex-wrap:wrap;">
          ${c.insNo?'<span>🏦 보험접수번호: '+c.insNo+'</span>':''}
          ${cIns?'<span>🏢 보험사: '+cIns.name+'</span>':''}
          ${c.adjuster?'<span>👤 손해사정: '+c.adjuster+'</span>':''}
          ${c.adjTel?'<span>📞 '+c.adjTel+'</span>':''}
        </div>`:''}
        ${c.desc?`<div style="font-size:12px;color:var(--tx2);margin-top:4px;">${c.desc.slice(0,50)}${c.desc.length>50?'...':''}</div>`:''}
        ${linkedBadges(linked,null)}
        <div style="display:flex;gap:6px;margin-top:8px;padding-top:6px;border-top:0.5px solid var(--bd3);">
          <button class="btn sm" data-mail-accident="${c.id}">📧 사고보고</button>
          <button class="btn sm" data-mail-insurance="${c.id}">📧 PL접수</button>
        </div>
      </div>`;
    });
  }
  ...
  // 소액클레임
  if(matchedMinor.length){
    html+=`<div style="font-size:13px;font-weight:600;color:var(--tx2);margin:0 0 8px;">소액클레임 (${matchedMinor.length}건)</div>`;
    matchedMinor.forEach(m=>{
      const linked=getLinked(m.claimRef,m.id);
      html+=`<div class="card" style="margin-bottom:10px;">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:5px;flex-wrap:wrap;">
          <span style="font-family:var(--mono);font-size:11px;color:var(--blue);cursor:pointer;" data-goto-minor="${m.id}">${m.id}</span>
          <span class="bdg b-접수">${m.status}</span>
          <span style="font-size:11px;color:var(--tx2);">${m.type||'-'}</span>
          <span style="font-weight:500;">${m.name||'-'}</span>
          <span style="font-size:11px;color:var(--tx2);margin-left:auto;">${m.adate||m.date||'-'}</span>
        </div>
        <div style="font-size:12px;color:var(--tx2);display:flex;gap:8px;flex-wrap:wrap;">
          ${m.logistics?`<span>📦 ${m.logistics}</span>`:''}
          ${m.lineteam?`<span>👥 ${m.lineteam}</span>`:''}
          ${m.tech?`<span>🔧 ${m.tech}</span>`:''}
          ${m.order?`<span>📋 ${m.order}</span>`:''}
        </div>
        ${linkedBadges(linked,m.claimRef)}
      </div>`;
    });
  }
  ...
  // 소비자원
  if(matchedCA.length){
    html+=`<div style="font-size:13px;font-weight:600;color:var(--tx2);margin:0 0 8px;">소비자원 (${matchedCA.length}건)</div>`;
    matchedCA.forEach(c=>{
      const relClaim=c.claimRef?claims.find(x=>x.id===c.claimRef):null;
      html+=`<div class="card" style="margin-bottom:10px;">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:5px;flex-wrap:wrap;">
          <span style="font-family:var(--mono);font-size:11px;color:var(--blue);cursor:pointer;" data-goto-ca="${c.id}">${c.no||'-'}</span>
          <span class="bdg b-접수">${c.status}</span>
          <span style="font-size:11px;background:#E6F1FB;color:var(--blue);padding:1px 6px;border-radius:99px;">${c.type}</span>
          <span style="font-weight:500;">${c.claimant||'-'}</span>
          <span style="font-size:11px;color:var(--tx2);margin-left:auto;">${c.date||'-'}</span>
        </div>
        <div style="font-size:12px;color:var(--tx2);">${c.desc?c.desc.slice(0,50):''}</div>
        <div style="font-size:12px;color:var(--tx2);margin-top:3px;display:flex;gap:8px;">
          ${c.claimAmount?`<span>청구: ${c.claimAmount.toLocaleString()}원</span>`:''}
          ${c.amount?`<span>결정: ${c.amount.toLocaleString()}원</span>`:''}
          ${relClaim?`<span style="cursor:pointer;" data-goto-claim="${relClaim.id}">📋 ${relClaim.id}</span>`:''}
        </div>
      </div>`;
    });
  }
  ...
  // 소송
  if(matchedSuits.length){
    html+=`<div style="font-size:13px;font-weight:600;color:var(--tx2);margin:0 0 8px;">소송 (${matchedSuits.length}건)</div>`;
    matchedSuits.forEach(s=>{
      const relClaim=s.claimRef?claims.find(x=>x.id===s.claimRef):null;
      html+=`<div class="card" style="margin-bottom:10px;">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:5px;flex-wrap:wrap;">
          <span style="font-family:var(--mono);font-size:11px;color:var(--red);cursor:pointer;" data-goto-suit="${s.id}">${s.no||'-'}</span>
          <span class="bdg b-소송중">${s.status||'-'}</span>
          <span style="font-size:11px;color:var(--tx2);">${s.type||'-'}</span>
          <span style="font-weight:500;">${s.plaintiff||'-'} \u2192 ${s.defendant||'-'}</span>
          <span style="font-size:11px;color:var(--tx2);margin-left:auto;">${s.date||'-'}</span>
        </div>
        <div style="font-size:12px;color:var(--tx2);display:flex;gap:8px;flex-wrap:wrap;">
          ${s.court?`<span>🏛 ${s.court} ${s.instance||''}</span>`:''}
          ${s.nextDate?`<span>📅 다음기일: ${s.nextDate}</span>`:''}
          ${s.amount?`<span>💰 소가: ${s.amount.toLocaleString()}원</span>`:''}
          ${s.result?`<span style="font-weight:500;color:${s.result==='승소'?'var(--green)':'var(--red)'};">⚖️ ${s.result}</span>`:''}
          ${relClaim?`<span style="cursor:pointer;" data-goto-claim="${relClaim.id}">📋 ${relClaim.id}</span>`:''}
        </div>
      </div>`;
    });
...
/* ══════════════════════════════════════════════
   메일 자동화 (mailto)
══════════════════════════════════════════════ */
function sendAccidentReportMail(claimId){
  const c=claims.find(x=>x.id===claimId);if(!c)return;
  const insName=(insCompanies||[]).find(i=>i.id===c.insCoId)?.name||'';
  const subject=encodeURIComponent(`[PL사고보고] ${c.id} ${c.name||''} ${c.type} 사고`);
  const body=encodeURIComponent([
    `[PL 사고 보고서]`,``,
    `■ 기본 정보`,
    `접수번호: ${c.id}`,`보험사 접수번호: ${c.insNo||'-'}`,`고객사: ${c.client||'-'}`,`담당 보험사: ${insName||'-'}`,``,
    `■ 고객 정보`,`고객명: ${c.name||'-'}`,`연락처: ${c.phone||'-'}`,`주소: ${c.addr||'-'}`,``,
    `■ 사고 정보`,`사고 유형: ${c.type||'-'}`,`대구분: ${c.groupName||'-'}`,`제품구분: ${c.pcatName||'-'}`,
    `설치일자: ${c.idate||'-'}`,`보험접수일: ${c.insDate||'-'}`,`설치기사: ${c.tname||'-'}`,`물류센터: ${c.logistics||'-'}`,``,
    `■ 피해 내용`,c.desc||'-',``,
    `■ 금액`,`추산보험금 O/S: ${c.amount?c.amount.toLocaleString()+'원':'-'}`,
    `조사비 O/S: ${c.survey?c.survey.toLocaleString()+'원':'-'}`,`지급보험금: ${c.finalPayment?c.finalPayment.toLocaleString()+'원':'-'}`,``,
    `■ 비고`,c.note||'-',``,
    `※ LX판토스 LMD/설치품질팀 클레임관리시스템 자동생성`,
  ].join('\n'));
  window.open(`mailto:?subject=${subject}&body=${body}`,'_blank');
}

function sendInsuranceMail(claimId){
  const c=claims.find(x=>x.id===claimId);if(!c)return;
  const ins=(insCompanies||[]).find(i=>i.id===c.insCoId);
  const insName=ins?.name||'';
  const to=encodeURIComponent(ins?.email||'');
  const subject=encodeURIComponent(`[PL접수요청] ${c.name||''} ${c.type} 사고 접수`);
  const body=encodeURIComponent([
    `안녕하세요, LX판토스 LMD/설치품질팀입니다.`,``,
    `아래와 같이 PL사고 보험 접수를 요청드립니다.`,``,
    `■ 접수 정보`,`당사 접수번호: ${c.id}`,`고객사: ${c.client||'-'}`,``,
    `■ 고객 정보`,`고객명: ${c.name||'-'}`,`연락처: ${c.phone||'-'}`,`주소: ${c.addr||'-'}`,``,
    `■ 사고 정보`,`사고 유형: ${c.type||'-'}`,`제품구분: ${c.pcatName||'-'}`,
    `설치일자: ${c.idate||'-'}`,`설치기사: ${c.tname||'-'}`,``,
    `■ 피해 내용`,c.desc||'-',``,
    `■ 추산 피해금액`,`추산보험금: ${c.amount?c.amount.toLocaleString()+'원':'미산정'}`,``,
    `확인 후 접수번호 회신 부탁드립니다.`,`감사합니다.`,``,
    `LX판토스 LMD/설치품질팀`,
  ].join('\n'));
...
function renderCAStats(){
  const el=$('ca-stat-grid');if(!el)return;
  const total=consumerCases.length;
  if(!total){el.innerHTML='';return;}
  const won=consumerCases.filter(c=>c.result&&(c.result.includes('성립')||c.result.includes('합의'))).length;
  const ratio=Math.round(won/total*100);
  const totalClaim=consumerCases.reduce((a,c)=>a+(c.claimAmount||0),0);
  const totalResult=consumerCases.reduce((a,c)=>a+(c.amount||0),0);
  el.innerHTML=`
    <div class="sc"><div class="sl">전체 건수</div><div class="sv bl">${total}건</div></div>
    <div class="sc"><div class="sl">인용률</div><div class="sv ${ratio>=50?'gr':'rd'}">${ratio}%</div></div>
    <div class="sc"><div class="sl">청구금액 합계</div><div class="sv am">${fmt만원(totalClaim)}</div></div>
    <div class="sc"><div class="sl">결정금액 합계</div><div class="sv gr">${fmt만원(totalResult)}</div></div>`;
}
...
function openCAEditPage(id){
  ...
  $('cae-f-no').value=c.no||'';
  $('cae-f-type').value=c.type||'피해구제';
  $('cae-f-claimant').value=c.claimant||'';
  $('cae-f-phone').value=c.phone||'';
  $('cae-f-respondent').value=c.respondent||'LX판토스';
  $('cae-f-date').value=c.date||'';
  $('cae-f-status').value=c.status||'접수';
  const fa=$('cae-f-asgn');if(fa)fa.innerHTML='<option value="">선택</option>'+(assignees||[]).map(a=>`<option ${c.assignee===a.name?'selected':''}>${a.name}</option>`).join('');
  $('cae-f-claim-amount').value=c.claimAmount||'';
  $('cae-f-amount').value=c.amount||'';
  $('cae-f-claimref').value=c.claimRef||'';
  $('cae-f-minorref').value=c.minorRef||'';
  $('cae-f-committee-date').value=c.committeeDate||'';
  $('cae-f-committee-place').value=c.committeePlace||'';
  $('cae-f-committee-date2').value=c.committeeDate2||'';
  $('cae-f-committee-place2').value=c.committeePlace2||'';
  $('cae-f-result').value=c.result||'';
  $('cae-f-desc').value=c.desc||'';
  $('cae-f-note').value=c.note||'';
  // 분쟁조정 필드 토글
  const isArb=c.type==='분쟁조정';
  ['cae-committee-wrap','cae-committee-place-wrap','cae-committee2-wrap','cae-committee2-place-wrap','cae-result-wrap'].forEach(id=>{
    const el=$(id);if(el)el.style.display=isArb?'':'none';
  });
  const typeEl=$('cae-f-type');
  if(typeEl)typeEl.onchange=()=>{
    const isArb2=typeEl.value==='분쟁조정';
    ['cae-committee-wrap','cae-committee-place-wrap','cae-committee2-wrap','cae-committee2-place-wrap','cae-result-wrap'].forEach(id=>{
      const el=$(id);if(el)el.style.display=isArb2?'':'none';
    });
  };
}
function saveCAEdit(){
  const c=curCADetail;if(!c)return;
  c.no=($('cae-f-no')||{}).value||c.no;
  c.claimant=($('cae-f-claimant')||{}).value||c.claimant;
  c.phone=($('cae-f-phone')||{}).value||c.phone;
  c.respondent=($('cae-f-respondent')||{}).value||c.respondent;
  c.date=($('cae-f-date')||{}).value||c.date;
  c.desc=($('cae-f-desc')||{}).value||c.desc;
  c.note=($('cae-f-note')||{}).value||'';
  c.updatedAt=new Date().toISOString().slice(0,10);
  persist();renderCAStats();renderCA();openCADetail(c.id);
}
...
function renderCA(){
  ...
  el.innerHTML=filtered.length?filtered.map(c=>`
    <div class="tr" style="grid-template-columns:110px 70px 1fr 80px 80px 70px 90px 90px;" data-open-ca="${c.id}">
      ...
      <span style="font-size:12px;">${c.result?`<span style="font-weight:500;color:var(--green);">${c.result}</span>`:''} ${c.amount?fmt만원(c.amount):'-'}</span>
    </div>`).join(''):'<div class="empty">소비자원 사건 없음</div>';
}
let _pendingCAHistory=null;
function openCAForm(id, prefill){
  curCAId=id||null;
  const c=id?consumerCases.find(x=>x.id===id):(prefill||{});
  // prefill에 history가 있으면 저장
  _pendingCAHistory=(prefill&&prefill.history&&!id)?prefill.history:null;
  $('ca-modal-title').textContent=id?'소비자원 사건 수정':'소비자원 사건 등록';
  const caTypeEl=$('ca-f-type');if(caTypeEl)caTypeEl.addEventListener('change',e=>toggleCAFields(e.target.value));
  const fa=$('ca-f-asgn');
  if(fa)fa.innerHTML='<option value="">선택</option>'+(assignees||[]).map(a=>`<option ${c.assignee===a.name?'selected':''}>${a.name}</option>`).join('');
  $('ca-f-no').value=c.no||'';
  const typeEl=$('ca-f-type');if(typeEl)typeEl.value=c.type||'피해구제';
  ...
  $('ca-f-amount').value=c.amount||'';
  $('ca-f-claimref').value=c.claimRef||'';
  const mnref=$('ca-f-minorref');if(mnref)mnref.value=c.minorRef||'';
  $('ca-f-desc').value=c.desc||'';$('ca-f-note').value=c.note||'';
  const fcla=$('ca-f-claim-amount');if(fcla)fcla.value=c.claimAmount||'';
  const cd=$('ca-f-committee-date');if(cd)cd.value=c.committeeDate||'';
  const cdp=$('ca-f-committee-place');if(cdp)cdp.value=c.committeePlace||'';
  const cd2=$('ca-f-committee-date2');if(cd2)cd2.value=c.committeeDate2||'';
  const cd2p=$('ca-f-committee-place2');if(cd2p)cd2p.value=c.committeePlace2||'';
  const cr=$('ca-f-result');if(cr)cr.value=c.result||'';
  toggleCAFields(c.type||'피해구제');
  $('ca-modal').classList.add('open');
}
function toggleCAFields(type){
  const isArb=type==='분쟁조정';
  ['ca-f-committee-wrap','ca-f-committee-place-wrap','ca-f-committee2-wrap','ca-f-committee2-place-wrap','ca-f-result-wrap'].forEach(id=>{
    const el=$(id);if(el)el.style.display=isArb?'':'none';
  });
}
...
function saveCA(){
  ...
  const d={
    ...
    claimRef:$('ca-f-claimref').value,
    minorRef:($('ca-f-minorref')||{}).value||'',
    assignee:($('ca-f-asgn')||{}).value||'',
    claimAmount:Number(($('ca-f-claim-amount')||{}).value)||0,
    committeeDate:($('ca-f-committee-date')||{}).value||'',
    committeePlace:($('ca-f-committee-place')||{}).value||'',
    committeeDate2:($('ca-f-committee-date2')||{}).value||'',
    committeePlace2:($('ca-f-committee-place2')||{}).value||'',
    result:($('ca-f-result')||{}).value||'',
    desc:$('ca-f-desc').value,note:$('ca-f-note').value,
    updatedAt:new Date().toISOString().slice(0,10)
  };
  // history 보존 (수정 시 기존 이력 유지)
  if(curCAId){
    const i=consumerCases.findIndex(x=>x.id===curCAId);
    if(i>=0){d.history=consumerCases[i].history||[];d.minorRef=d.minorRef||consumerCases[i].minorRef||'';consumerCases[i]=d;}
  }else{
    // prefill에서 넘어온 history 우선 사용
    d.history=_pendingCAHistory||[{date:new Date().toISOString().slice(0,10),text:`${d.type} 접수`}];
    _pendingCAHistory=null;
    consumerCases.unshift(d);
  }
...
function openCADetail(id){
  ...
  const caBtn=document.querySelector('.nb[data-fn="navConsumer"]');
  if(caBtn)caBtn.classList.add('active');
  $('ca-detail-title').textContent=(c.no||'사건번호 미입력')+' - '+c.type;

  const typeOrder=['자율조정','피해구제','분쟁조정'];
  const curTypeIdx=typeOrder.indexOf(c.type);
  const stepsMap={
    '자율조정':['접수','조사','합의','종결','취하'],
    '피해구제':['접수','조사','합의','결정','종결','취하'],
    '분쟁조정':['접수','조사','1차위원회','2차위원회','결정','종결','취하'],
  };
  const steps=stepsMap[c.type]||['접수','조사','결정','종결'];
  const si=Math.max(steps.indexOf(c.status),0);
  const claimAmt=c.claimAmount||0;
  const resultAmt=c.amount||0;
  const ratio=claimAmt>0?Math.round(resultAmt/claimAmt*100):null;
  const isEnded=c.status==='종결'||c.status==='취하';
  const nextType=typeOrder[curTypeIdx+1];
  ...
  function renderHistory(history){
    ...
    history.forEach(h=>{
      if(h.stage){
        html+=`<div style="display:flex;align-items:center;gap:6px;margin:10px 0 6px;"><div style="height:1px;flex:1;background:var(--bd3);"></div><span style="font-size:11px;font-weight:600;color:var(--blue);white-space:nowrap;padding:0 6px;">${h.stage}</span><div style="height:1px;flex:1;background:var(--bd3);"></div></div>`;
      } else {
        html+=`<div class="tl-i"><div class="tl-d"></div><div class="tl-ln"></div><div><div style="font-size:13px;">${h.text||''}</div><div class="tl-dt">${h.date||''}</div></div></div>`;
      }
    });
    return html;
  }

  $('ca-detail-content').innerHTML=`
    <!-- 유형 타임라인 -->
    <div style="display:flex;align-items:center;margin-bottom:16px;background:var(--bg2);border-radius:var(--r-lg);padding:12px 20px;">
      ${typeOrder.map((t,i)=>`
        <div style="display:flex;align-items:center;flex:1;">
          <div style="display:flex;flex-direction:column;align-items:center;gap:4px;flex:1;">
            <div style="width:30px;height:30px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;
              background:${i<curTypeIdx?'var(--green)':i===curTypeIdx?'var(--blue)':'var(--bg1)'};
              color:${i<=curTypeIdx?'#fff':'var(--tx3)'};
              border:2px solid ${i<curTypeIdx?'var(--green)':i===curTypeIdx?'var(--blue)':'var(--bd3)'};">
              ${i<curTypeIdx?'✓':i+1}
            </div>
            <span style="font-size:11px;font-weight:${i===curTypeIdx?'700':'400'};color:${i===curTypeIdx?'var(--blue)':i<curTypeIdx?'var(--green)':'var(--tx3)'};">${t}</span>
          </div>
          ${i<typeOrder.length-1?`<div style="height:2px;flex:0 0 32px;background:${i<curTypeIdx?'var(--green)':'var(--bd3)'};margin-bottom:18px;"></div>`:''}
        </div>`).join('')}
    </div>
    <!-- 진행상태 단계 -->
    <div class="ps" style="overflow-x:auto;margin-bottom:14px;">${steps.map((s,i)=>`<div class="pst ${i<si?'done':i===si?'active':''}"><div class="pc">${i<si?'✓':i+1}</div><div class="pl" style="font-size:10px;">${s}</div></div>`).join('')}</div>
    <div class="dg">
      <div class="card">
        <h3>사건 정보</h3>
        <div class="dr"><span class="dk">사건번호</span><span style="font-family:var(--mono);font-weight:500;">${c.no||'-'}</span></div>
        <div class="dr"><span class="dk">유형</span><span class="ca-${c.type}">${c.type}</span></div>
        <div class="dr"><span class="dk">신청인</span><span style="font-weight:500;">${c.claimant||'-'}</span></div>
        <div class="dr"><span class="dk">연락처</span><span>${c.phone||'-'}</span></div>
        <div class="dr"><span class="dk">피신청인</span><span>${c.respondent||'-'}</span></div>
        <div class="dr"><span class="dk">신청일</span><span>${c.date||'-'}</span></div>
        <div class="dr"><span class="dk">담당자</span><span>${c.assignee||'-'}</span></div>
        ${c.claimRef?`<div class="dr"><span class="dk">연결 클레임</span><span style="color:var(--blue);font-family:var(--mono);font-size:12px;">${c.claimRef}</span></div>`:''}
        ${c.minorRef?`<div class="dr"><span class="dk">소액클레임</span><span style="color:var(--blue);font-family:var(--mono);font-size:12px;">${c.minorRef}</span></div>`:''}
        <div style="border-top:0.5px solid var(--bd3);margin:9px 0;"></div>
        <h3>금액</h3>
        ...
        <div style="border-top:0.5px solid var(--bd3);margin:9px 0;"></div>
        <h3>분쟁조정위원회</h3>
        <div class="dr"><span class="dk">1차 개최일</span><span>${c.committeeDate||'-'}</span></div>
        ${c.committeePlace?`<div class="dr"><span class="dk">1차 장소</span><span>${c.committeePlace}</span></div>`:''}
        ${c.committeeDate2?`<div class="dr"><span class="dk">2차 개최일</span><span>${c.committeeDate2}</span></div>`:''}
        ${c.committeePlace2?`<div class="dr"><span class="dk">2차 장소</span><span>${c.committeePlace2}</span></div>`:''}
        <div class="dr"><span class="dk">조정 결과</span><span style="font-weight:500;">${c.result||'미확정'}</span></div>`:''}
        ${isEnded&&nextType?`
        <div style="border-top:0.5px solid var(--bd3);margin:9px 0;"></div>
        <div style="padding:10px;background:#E6F1FB;border-radius:var(--r-md);">
          <div style="font-size:12px;color:var(--blue);font-weight:500;margin-bottom:6px;">${c.type} ${c.status} - ${nextType}로 이관</div>
          <div style="font-size:12px;color:var(--tx2);margin-bottom:8px;">현재 이력을 유지하며 ${nextType} 단계로 진행합니다.</div>
          <button class="btn sm pri" id="btn-ca-escalate">\u2192 ${nextType}로 이관</button>
        </div>`:''}
      </div>
      <div class="card">
        <h3>피해 내용</h3>
        <p style="font-size:13px;line-height:1.7;">${c.desc||'-'}</p>
        ${c.note?`<div style="padding:7px 9px;background:var(--bg2);border-radius:var(--r-md);font-size:12px;color:var(--tx2);margin-top:6px;"><b>비고:</b> ${c.note}</div>`:''}
        <div style="border-top:0.5px solid var(--bd3);margin:12px 0;"></div>
        <h3>전체 처리 이력</h3>
        <div id="ca-history-list">${renderHistoryEditable(c.history,()=>{persist();openCADetail(c.id);})}</div>
        <div style="display:flex;gap:7px;margin-top:9px;">
          <input type="text" id="ca-hist-inp" placeholder="처리 내용 입력..." style="flex:1;padding:5px 8px;font-size:12px;border:0.5px solid var(--bd2);border-radius:var(--r-md);background:var(--bg1);color:var(--tx1);font-family:var(--font);">
          <input type="date" id="ca-hist-date" style="padding:5px 8px;font-size:12px;border:0.5px solid var(--bd2);border-radius:var(--r-md);background:var(--bg1);color:var(--tx1);font-family:var(--font);">
          <button class="btn sm" id="btn-ca-add-hist">추가</button>
        </div>
        ${c.type==='분쟁조정'?`
        <div style="margin-top:9px;padding:9px;background:var(--bg2);border-radius:var(--r-md);">
          <div style="font-size:12px;font-weight:500;color:var(--tx2);margin-bottom:6px;">분쟁조정 결과 입력</div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;">
            <div><label style="font-size:11px;color:var(--tx2);">결정일자</label><input type="date" id="ca-arb-date" style="width:100%;padding:4px 7px;font-size:12px;border:0.5px solid var(--bd2);border-radius:var(--r-md);background:var(--bg1);color:var(--tx1);font-family:var(--font);margin-top:2px;box-sizing:border-box;"></div>
            <div><label style="font-size:11px;color:var(--tx2);">조정결과</label><select id="ca-arb-result" style="width:100%;padding:4px 7px;font-size:12px;border:0.5px solid var(--bd2);border-radius:var(--r-md);background:var(--bg1);color:var(--tx1);font-family:var(--font);margin-top:2px;"><option value="">선택</option><option>조정성립</option><option>조정불성립</option><option>각하</option><option>취하</option></select></div>
            <div><label style="font-size:11px;color:var(--tx2);">결정금액 (원)</label><input type="number" id="ca-arb-amount" placeholder="0" style="width:100%;padding:4px 7px;font-size:12px;border:0.5px solid var(--bd2);border-radius:var(--r-md);background:var(--bg1);color:var(--tx1);font-family:var(--font);margin-top:2px;box-sizing:border-box;"></div>
            <div><label style="font-size:11px;color:var(--tx2);">인용률 (%)</label><input type="number" id="ca-arb-ratio" placeholder="0" style="width:100%;padding:4px 7px;font-size:12px;border:0.5px solid var(--bd2);border-radius:var(--r-md);background:var(--bg1);color:var(--tx1);font-family:var(--font);margin-top:2px;box-sizing:border-box;"></div>
          </div>
          <textarea id="ca-arb-note" placeholder="결정 요지..." rows="2" style="width:100%;padding:5px 8px;font-size:12px;border:0.5px solid var(--bd2);border-radius:var(--r-md);background:var(--bg1);color:var(--tx1);font-family:var(--font);margin-top:6px;box-sizing:border-box;resize:vertical;"></textarea>
          <button class="btn sm pri" id="btn-ca-arb-save" style="width:100%;margin-top:6px;">결과 저장</button>
        </div>`:''}
        ${c.type!=='분쟁조정'?`
        <div style="margin-top:9px;padding:9px;background:var(--bg2);border-radius:var(--r-md);">
          <div style="font-size:12px;font-weight:500;color:var(--tx2);margin-bottom:6px;">${c.type} 결과 입력</div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;">
            <div><label style="font-size:11px;color:var(--tx2);">결정일자</label><input type="date" id="ca-res-date" style="width:100%;padding:4px 7px;font-size:12px;border:0.5px solid var(--bd2);border-radius:var(--r-md);background:var(--bg1);color:var(--tx1);font-family:var(--font);margin-top:2px;box-sizing:border-box;"></div>
            <div><label style="font-size:11px;color:var(--tx2);">결과</label><select id="ca-res-result" style="width:100%;padding:4px 7px;font-size:12px;border:0.5px solid var(--bd2);border-radius:var(--r-md);background:var(--bg1);color:var(--tx1);font-family:var(--font);margin-top:2px;"><option value="">선택</option><option>합의성립</option><option>합의불성립</option><option>취하</option><option>종결</option></select></div>
            <div><label style="font-size:11px;color:var(--tx2);">합의금액 (원)</label><input type="number" id="ca-res-amount" placeholder="0" style="width:100%;padding:4px 7px;font-size:12px;border:0.5px solid var(--bd2);border-radius:var(--r-md);background:var(--bg1);color:var(--tx1);font-family:var(--font);margin-top:2px;box-sizing:border-box;"></div>
            <div><label style="font-size:11px;color:var(--tx2);">인용률 (%)</label><input type="number" id="ca-res-ratio" placeholder="0" style="width:100%;padding:4px 7px;font-size:12px;border:0.5px solid var(--bd2);border-radius:var(--r-md);background:var(--bg1);color:var(--tx1);font-family:var(--font);margin-top:2px;box-sizing:border-box;"></div>
          </div>
        ...
        <div style="margin-top:9px;display:flex;gap:5px;align-items:center;flex-wrap:wrap;">
          ...
          <select id="ca-st-sel" style="padding:4px 7px;font-size:12px;border:0.5px solid var(--bd2);border-radius:var(--r-md);background:var(--bg1);color:var(--tx1);font-family:var(--font);">
            ${steps.map(s=>`<option ${c.status===s?'selected':''}>${s}</option>`).join('')}
          </select>
          <button class="btn sm pri" id="btn-ca-chg-status">적용</button>
        </div>
      </div>
    </div>
    `+buildCALinkedHtml(c.minorRef||null,c.claimRef||null);
  ...
  if(bah)bah.addEventListener('click',()=>{
    const inp=$('ca-hist-inp');if(!inp||!inp.value.trim())return;
    const dt=($('ca-hist-date')||{}).value||new Date().toISOString().slice(0,10);
    if(!c.history)c.history=[];
    c.history.push({date:dt,text:inp.value.trim()});
    inp.value='';
    persist();openCADetail(c.id);
  });
  // 분쟁조정 결과 저장
  const carbSave=$('btn-ca-arb-save');
  if(carbSave)carbSave.addEventListener('click',()=>{
    const date=($('ca-arb-date')||{}).value||new Date().toISOString().slice(0,10);
    const result=($('ca-arb-result')||{}).value||'';
    const amount=Number(($('ca-arb-amount')||{}).value)||0;
    const ratio=Number(($('ca-arb-ratio')||{}).value)||0;
    const note=($('ca-arb-note')||{}).value||'';
    if(!c.history)c.history=[];
    let text=`분쟁조정 결과: ${result||'미확정'}`;
    if(amount)text+=`\n결정금액: ${amount.toLocaleString()}원`;
    if(ratio)text+=`\n인용률: ${ratio}%`;
    if(note)text+=`\n${note}`;
  ...
  // 자율조정/피해구제 결과 저장
  const cresSave=$('btn-ca-res-save');
  if(cresSave)cresSave.addEventListener('click',()=>{
    const date=($('ca-res-date')||{}).value||new Date().toISOString().slice(0,10);
    const result=($('ca-res-result')||{}).value||'';
    const amount=Number(($('ca-res-amount')||{}).value)||0;
    const ratio=Number(($('ca-res-ratio')||{}).value)||0;
    const note=($('ca-res-note')||{}).value||'';
    if(!c.history)c.history=[];
    let text=`${c.type} 결과: ${result||'미확정'}`;
    if(amount)text+=`\n합의금액: ${amount.toLocaleString()}원`;
    if(ratio)text+=`\n인용률: ${ratio}%`;
    if(note)text+=`\n${note}`;
    c.history.push({date,text,result,amount,ratio});
    if(result)c.result=result;
    ...
    if((result==='합의불성립'||result==='조정불성립')&&nextType){
      c.status='취하';
      persist();
      // 자동 이관 확인
      const doEscalate=()=>{
        const today=new Date().toISOString().slice(0,10);
        c.history.push({date:today,text:c.type+' 불성립 > '+nextType+'으로 이관'});
        c.type=nextType;c.status='접수';
        if(!c.history)c.history=[];
        c.history.push({date:today,text:`${nextType} 접수`});
        persist();openCADetail(c.id);
      };
      // 이관 확인 배너 표시
      const msg=document.createElement('div');
      msg.style.cssText='position:fixed;bottom:24px;right:24px;padding:14px 18px;background:var(--blue);color:#fff;border-radius:10px;font-size:13px;z-index:400;box-shadow:0 4px 16px rgba(0,0,0,0.2);';
      msg.innerHTML=`${c.type} 불성립 - <b>${nextType}</b>으로 이관하시겠습니까? <button style="margin-left:10px;padding:4px 10px;background:#fff;color:var(--blue);border:none;border-radius:6px;cursor:pointer;font-weight:600;" id="btn-ca-escalate-confirm">이관</button> <button style="margin-left:6px;padding:4px 10px;background:rgba(255,255,255,0.2);color:#fff;border:none;border-radius:6px;cursor:pointer;" id="btn-ca-escalate-cancel">취소</button>`;
      document.body.appendChild(msg);
      document.getElementById('btn-ca-escalate-confirm').addEventListener('click',()=>{msg.remove();doEscalate();});
      document.getElementById('btn-ca-escalate-cancel').addEventListener('click',()=>{msg.remove();openCADetail(c.id);});
      return;
    }
    persist();openCADetail(c.id);
  });
  const bcs=$('btn-ca-chg-status');
  if(bcs)bcs.addEventListener('click',()=>{
    const sel=$('ca-st-sel');if(!sel)return;
    const oldSt=c.status;c.status=sel.value;
    if(oldSt!==sel.value){
      if(!c.history)c.history=[];
      c.history.push({date:new Date().toISOString().slice(0,10),text:`상태 변경: ${oldSt} \u2192 ${sel.value}`});
    }
    persist();openCADetail(c.id);
  });
  // 유형 이관 버튼
  const besc=$('btn-ca-escalate');
  if(besc)besc.addEventListener('click',()=>{
    const today=new Date().toISOString().slice(0,10);
    if(!c.history)c.history=[];
    // 이관 구분선 추가
    c.history.push({stage:`${c.type} 종결 \u2192 ${nextType} 이관`,date:today});
    c.history.push({date:today,text:`${nextType} 단계 접수`});
    c.type=nextType;c.status='접수';
    persist();openCADetail(c.id);
  });
}

function deleteCA(id){consumerCases=consumerCases.filter(x=>x.id!==id);curCADetail=null;persist();renderCAStats();nav('consumer',document.querySelector('.nb[data-fn="navConsumer"]'));}
...
function openSuitDetail(id){
  ...
  const suitBtn=document.querySelector('.nb[data-fn="navLawsuit"]');
  if(suitBtn)suitBtn.classList.add('active');
  $('suit-detail-title').textContent=s.no||'사건번호 미입력';

  const steps=['소장접수','변론','판결','항소','조정','확정','취하'];
  const si=Math.max(steps.indexOf(s.status),0);

  $('suit-detail-content').innerHTML=`
    <div class="ps" style="overflow-x:auto;margin-bottom:14px;">${steps.map((st,i)=>`<div class="pst ${i<si?'done':i===si?'active':''}"><div class="pc">${i<si?'<svg class="ico ico-md"><use href="#ico-check"/></svg>':i+1}</div><div class="pl" style="font-size:10px;">${st}</div></div>`).join('')}</div>
    <div class="dg">
      <div class="card">
        <h3>사건 정보</h3>
        <div class="dr"><span class="dk">사건번호</span><span style="font-family:var(--mono);font-weight:500;">${s.no||'-'}</span></div>
        <div class="dr"><span class="dk">관할법원</span><span>${s.court||'-'}</span></div>
        <div class="dr"><span class="dk">심급</span><span>${s.instance||'-'}</span></div>
        <div class="dr"><span class="dk">소송유형</span><span>${s.type||'-'}</span></div>
        <div class="dr"><span class="dk">원고</span><span>${s.plaintiff||'-'}</span></div>
        <div class="dr"><span class="dk">피고</span><span>${s.defendant||'-'}</span></div>
        <div class="dr"><span class="dk">소장접수일</span><span>${s.date||'-'}</span></div>
        <div class="dr"><span class="dk">다음 기일</span><span style="font-weight:500;color:var(--blue);">${s.nextDate||'-'}</span></div>
        <div class="dr"><span class="dk">담당 변호사</span><span>${s.lawyer||'-'}</span></div>
        ${s.claimRef?`<div class="dr"><span class="dk">연결 클레임</span><span style="color:var(--blue);font-family:var(--mono);font-size:12px;">${s.claimRef}</span></div>`:''}
        <div style="border-top:0.5px solid var(--bd3);margin:9px 0;"></div>
        <h3>금액 및 결과</h3>
      ...
      <div class="card">
        <h3>사건 개요</h3>
        <p style="font-size:13px;line-height:1.7;">${s.desc||'-'}</p>
        ${s.note?`<div style="padding:7px 9px;background:var(--bg2);border-radius:var(--r-md);font-size:12px;color:var(--tx2);margin-top:6px;"><b style="font-weight:500;">진행경과:</b> ${s.note}</div>`:''}
        <div style="border-top:0.5px solid var(--bd3);margin:12px 0;"></div>
        <h3>처리 이력</h3>
        <div id="suit-history-list">${renderHistoryEditable(s.history||[],()=>{persist();openSuitDetail(s.id);})}</div>
        <div style="display:flex;gap:7px;margin-top:9px;">
          <input type="text" id="suit-hist-inp" placeholder="처리 내용..." style="flex:1;padding:5px 8px;font-size:12px;border:0.5px solid var(--bd2);border-radius:var(--r-md);background:var(--bg1);color:var(--tx1);font-family:var(--font);">
          <input type="date" id="suit-hist-date" style="padding:5px 8px;font-size:12px;border:0.5px solid var(--bd2);border-radius:var(--r-md);background:var(--bg1);color:var(--tx1);font-family:var(--font);">
          <button class="btn sm" id="btn-suit-add-hist">추가</button>
        </div>
        <!-- 단계별 구조화 이력 -->
        <div style="margin-top:9px;border-top:0.5px solid var(--bd3);padding-top:9px;">
          <div style="font-size:12px;font-weight:500;color:var(--tx2);margin-bottom:6px;">단계별 등록</div>
          <select id="suit-stage-sel" style="width:100%;padding:5px 8px;font-size:12px;border:0.5px solid var(--bd2);border-radius:var(--r-md);background:var(--bg1);color:var(--tx1);font-family:var(--font);margin-bottom:6px;">
            <option value="">-- 단계 선택 --</option>
            <option value="소장접수">소장접수</option>
            <option value="보험접수">보험접수</option>
            <option value="피제소대응품의">피제소대응품의</option>
            <option value="인장날인">인장날인</option>
            <option value="변론">변론</option>
            <option value="판결">판결</option>
            <option value="조정">조정</option>
            <option value="항소">항소</option>
          </select>
          <div id="suit-stage-fields"></div>
          <button class="btn sm pri" id="btn-suit-stage-add" style="width:100%;margin-top:6px;">단계 이력 추가</button>
        </div>
        <div style="margin-top:9px;display:flex;gap:5px;align-items:center;flex-wrap:wrap;">
          <span style="font-size:12px;color:var(--tx2);">상태:</span>
          <select id="suit-st-sel" style="padding:4px 7px;font-size:12px;border:0.5px solid var(--bd2);border-radius:var(--r-md);background:var(--bg1);color:var(--tx1);font-family:var(--font);">
            ${steps.map(st=>`<option ${s.status===st?'selected':''}>${st}</option>`).join('')}
          </select>
          <button class="btn sm pri" id="btn-suit-chg-status">적용</button>
        </div>
      </div>
    </div>
    `+buildSuitLinkedHtml(s.claimRef||null);

  bindHistoryEditable(s.history||[],'#suit-history-list',()=>{persist();openSuitDetail(s.id);});
  const bah=$('btn-suit-add-hist');
  if(bah)bah.addEventListener('click',()=>{
    const inp=$('suit-hist-inp');if(!inp||!inp.value.trim())return;
    const dt=($('suit-hist-date')||{}).value||new Date().toISOString().slice(0,10);
    if(!s.history)s.history=[];
    s.history.push({date:dt,text:inp.value.trim()});
    inp.value='';
    persist();openSuitDetail(s.id);
  });
  // 단계별 등록
  const stageSel=$('suit-stage-sel');
  if(stageSel)stageSel.addEventListener('change',()=>{
    const stage=stageSel.value;
    const sf=$('suit-stage-fields');if(!sf)return;
    const IS='width:100%;padding:5px 8px;font-size:12px;border:0.5px solid var(--bd2);border-radius:var(--r-md);background:var(--bg1);color:var(--tx1);font-family:var(--font);margin-bottom:5px;box-sizing:border-box;';
    function mkIn(type,id,ph){return '<input type="'+type+'" id="'+id+'" placeholder="'+(ph||'')+'" style="'+IS+'">';}
    function mkSel(id,opts){return '<select id="'+id+'" style="'+IS+'">'+opts.map(o=>'<option>'+o+'</option>').join('')+'</select>';}
    let fhtml='';
    switch(stage){
      case '소장접수': fhtml=mkIn('date','sf-date','')+mkIn('text','sf-route','접수경로 (예: 법무팀 통해 메일로 전달)')+mkIn('text','sf-note','추가 내용'); break;
      case '보험접수': fhtml=mkIn('date','sf-date','')+mkIn('text','sf-note','보험사명, 접수번호 등'); break;
      case '피제소대응품의': fhtml=mkIn('date','sf-date','')+mkIn('text','sf-note','품의 내용'); break;
      case '인장날인': fhtml=mkIn('date','sf-date','')+mkIn('text','sf-firm','법무법인명')+mkIn('text','sf-note','추가 내용'); break;
      case '변론': fhtml=mkIn('date','sf-date','')+mkIn('text','sf-court','법원 및 법정 (예: 서울지법 202호)')+mkIn('text','sf-time','시간 (예: 14:00)')+mkIn('text','sf-note','변론 내용'); break;
      case '판결': fhtml=mkIn('date','sf-date','')+mkSel('sf-result',['판결결과 선택','승소','일부승소','패소'])+mkIn('number','sf-amount','판결금액 (원)')+mkIn('number','sf-ratio','승소율 (%)')+mkIn('text','sf-dept','담당재판부')+mkIn('text','sf-note','판결 요지'); break;
      case '조정': fhtml=mkIn('date','sf-date','')+mkSel('sf-result',['조정결과 선택','조정성립','조정불성립'])+mkIn('number','sf-amount','조정금액 (원)')+mkIn('number','sf-ratio','소가 대비 조정율 (%)')+mkIn('text','sf-note','조정 내용'); break;
      case '항소': fhtml=mkIn('date','sf-date','')+mkSel('sf-appellant',['항소인 선택','원고(당사)','피고(상대방)'])+mkIn('text','sf-court','항소법원')+mkIn('text','sf-note','항소 이유 요약'); break;
    }
    sf.innerHTML=fhtml;
  });
  const stageAdd=$('btn-suit-stage-add');
  if(stageAdd)stageAdd.addEventListener('click',()=>{
    const stage=($('suit-stage-sel')||{}).value||'';if(!stage)return;
    const date=($('sf-date')||{}).value||new Date().toISOString().slice(0,10);
    const note=($('sf-note')||{}).value||'';
    const result=($('sf-result')||{}).value||'';
    const amount=($('sf-amount')||{}).value||'';
    const ratio=($('sf-ratio')||{}).value||'';
    const route=($('sf-route')||{}).value||'';
    const court=($('sf-court')||{}).value||'';
    const time=($('sf-time')||{}).value||'';
    const firm=($('sf-firm')||{}).value||'';
    const dept=($('sf-dept')||{}).value||'';
    const appellant=($('sf-appellant')||{}).value||'';
    // 텍스트 조합
    const lines=[stage];
    if(route)lines.push('접수경로: '+route);
    if(firm)lines.push('법무법인: '+firm);
    if(court)lines.push('장소: '+court+(time?' '+time:''));
    if(result)lines.push('결과: '+result);
    if(amount)lines.push('금액: '+Number(amount).toLocaleString()+'원');
    if(ratio)lines.push('비율: '+ratio+'%');
    if(dept)lines.push('담당재판부: '+dept);
    if(appellant)lines.push('항소인: '+appellant);
  ...
  if(bcs)bcs.addEventListener('click',()=>{
    const sel=$('suit-st-sel');if(!sel)return;
    const old=s.status;s.status=sel.value;
    if(old!==sel.value){
      if(!s.history)s.history=[];
      s.history.push({date:new Date().toISOString().slice(0,10),text:'상태 변경: '+old+' > '+sel.value});
    }
    persist();openSuitDetail(s.id);
  });
}

function openSuitEditPage(){
  const s=curSuitDetail;if(!s)return;
  document.querySelectorAll('.sec').forEach(sec=>sec.classList.remove('active'));
  $('s-suit-edit').classList.add('active');
  $('suit-edit-title').textContent='소송 수정 - '+(s.no||'');
  $('se-f-no').value=s.no||'';
  $('se-f-court').value=s.court||'';
  $('se-f-instance').value=s.instance||'1심';
  $('se-f-type').value=s.type||'구상금청구';
  $('se-f-plaintiff').value=s.plaintiff||'';
  $('se-f-defendant').value=s.defendant||'LX판토스';
  $('se-f-amount').value=s.amount||'';
  $('se-f-date').value=s.date||'';
  $('se-f-status').value=s.status||'소장접수';
  $('se-f-nextdate').value=s.nextDate||'';
  $('se-f-lawyer').value=s.lawyer||'';
  $('se-f-claimref').value=s.claimRef||'';
  $('se-f-result-amount').value=s.resultAmount||'';
  $('se-f-result').value=s.result||'';
  $('se-f-desc').value=s.desc||'';
  $('se-f-note').value=s.note||'';
}
function saveSuitEdit(){
  const s=curSuitDetail;if(!s)return;
  s.no=$('se-f-no').value;s.court=$('se-f-court').value;
  s.instance=$('se-f-instance').value;s.type=$('se-f-type').value;
  s.plaintiff=$('se-f-plaintiff').value;s.defendant=$('se-f-defendant').value;
  s.amount=Number($('se-f-amount').value)||0;s.date=$('se-f-date').value;
  s.status=$('se-f-status').value;s.nextDate=$('se-f-nextdate').value;
  s.lawyer=$('se-f-lawyer').value;s.claimRef=$('se-f-claimref').value;
  s.resultAmount=Number($('se-f-result-amount').value)||0;s.result=$('se-f-result').value;
  s.desc=$('se-f-desc').value;s.note=$('se-f-note').value;
  s.updatedAt=new Date().toISOString().slice(0,10);
  persist();renderSuit();openSuitDetail(s.id);
}

function deleteSuit(id){
  if(!confirm('소송 사건을 삭제하시겠습니까?'))return;
  lawsuits=lawsuits.filter(x=>x.id!==id);
  curSuitDetail=null;persist();
  nav('lawsuit',document.querySelector('.nb[data-fn="navLawsuit"]'));
}
function renderSuit(){
