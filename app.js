...
function linkMinorToCA(minorId){
  ...
  const prefill={
    ...
    type:'자율조정',status:'접수',
    history:[...(m.history||[]),{date:new Date().toISOString().slice(0,10),text:'소비자원 접수'}],
  };
  // 소비자원 탭으로 이동 후 모달 열기
  document.querySelectorAll('.sec').forEach(s=>s.classList.remove('active'));
  document.querySelectorAll('.nb').forEach(b=>b.classList.remove('active'));
...
function openCADetail(id){
  ...
  $('ca-detail-content').innerHTML=`
    <!-- 유형 타임라인 -->
    <div style="display:flex;align-items:center;gap:0;margin-bottom:16px;background:var(--bg2);border-radius:var(--r-lg);padding:10px 16px;">
      ${typeOrder.map((t,i)=>`
        <div style="display:flex;align-items:center;gap:0;flex:1;">
          <div style="display:flex;flex-direction:column;align-items:center;gap:4px;flex:1;">
            <div style="width:28px;height:28px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:600;
              ...
              ${i<curTypeIdx?'<svg style="width:14px;height:14px;stroke:#fff;fill:none;stroke-width:2.5;stroke-linecap:round;stroke-linejoin:round;"><polyline points="20 6 9 17 4 12" transform="scale(0.6) translate(2,2)"/></svg>':i+1}
            </div>
            <span style="font-size:11px;font-weight:${i===curTypeIdx?'600':'400'};color:${i===curTypeIdx?'var(--blue)':i<curTypeIdx?'var(--green)':'var(--tx3)'};">${t}</span>
          </div>
          ${i<typeOrder.length-1?`<div style="height:2px;width:40px;background:${i<curTypeIdx?'var(--green)':'var(--bd3)'};margin-bottom:16px;flex-shrink:0;"></div>`:''}
        </div>`).join('')}
    </div>
    <!-- 진행상태 단계 -->
    <div class="ps" style="overflow-x:auto;margin-bottom:14px;">${steps.map((s,i)=>`<div class="pst ${i<si?'done':i===si?'active':''}"><div class="pc">${i<si?'<svg class="ico ico-md"><use href="#ico-check"/></svg>':i+1}</div><div class="pl">${s}</div></div>`).join('')}</div>
    <div class="dg">
      <div class="card">
        <h3>사건 정보</h3>
        <div class="dr"><span class="dk">사건번호</span><span style="font-family:var(--mono);font-weight:500;">${c.no||'-'}</span></div>
        <div class="dr"><span class="dk">유형</span><span><span class="ca-${c.type}">${c.type}</span></span></div>
        ...
        ${c.minorRef?`<div class="dr"><span class="dk">연결 소액클레임</span><span style="color:var(--blue);font-family:var(--mono);font-size:12px;">${c.minorRef}</span></div>`:''}
        <div style="border-top:0.5px solid var(--bd3);margin:9px 0;"></div>
        <h3>금액</h3>
      ...
      <div class="card">
        <h3>피해 내용</h3>
        <p style="font-size:13px;line-height:1.7;">${c.desc||'-'}</p>
        ${c.note?`<div style="padding:7px 9px;background:var(--bg2);border-radius:var(--r-md);font-size:12px;color:var(--tx2);margin-top:6px;"><b style="font-weight:500;">비고:</b> ${c.note}</div>`:''}
        <div style="border-top:0.5px solid var(--bd3);margin:12px 0;"></div>
        <h3>처리 이력</h3>
        <div>${(c.history||[]).map(h=>`<div class="tl-i"><div class="tl-d"></div><div class="tl-ln"></div><div><div style="font-size:13px;">${h.text}</div><div class="tl-dt">${h.date}</div></div></div>`).join('')||'<div style="font-size:12px;color:var(--tx3);padding:4px 0;">이력 없음</div>'}</div>
        <div style="display:flex;gap:7px;margin-top:9px;">
          <input type="text" id="ca-hist-inp" placeholder="처리 내용..." style="flex:1;padding:5px 8px;font-size:12px;border:0.5px solid var(--bd2);border-radius:var(--r-md);background:var(--bg1);color:var(--tx1);font-family:var(--font);">
  ...
  if(bcs)bcs.addEventListener('click',()=>{
    ...
    const old=c.status;c.status=sel.value;
    if(old!==sel.value){
      if(!c.history)c.history=[];
      c.history.push({date:new Date().toISOString().slice(0,10),text:`상태 변경: ${old} → ${sel.value}`});
    }
    persist();openCADetail(c.id);
  });
}
