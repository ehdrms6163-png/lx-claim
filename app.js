...
function _initHandlers(){
  ...
  const caedit=$('btn-ca-edit');if(caedit)caedit.addEventListener('click',()=>{if(curCADetail)openCAForm(curCADetail.id);});
  const cadel=$('btn-ca-delete');if(cadel)cadel.addEventListener('click',()=>{if(curCADetail)deleteCA(curCADetail.id);});if(mnback)mnback.addEventListener('click',()=>nav('minor',document.querySelector('.nb[data-fn="navMinor"]')));
...
function openCADetail(id){
  ...
  $('ca-detail-content').innerHTML=`
    <div class="ps" style="overflow-x:auto;">${steps.map((s,i)=>`<div class="pst ${i<si?'done':i===si?'active':''}"><div class="pc">${i<si?'<svg class="ico ico-md"><use href="#ico-check"/></svg>':i+1}</div><div class="pl">${s}</div></div>`).join('')}</div>
    <div class="dg">
      <div class="card">
        <h3>사건 정보</h3>
    ...
    ${(()=>{
      ...
      if(!total)return '';
      const won=consumerCases.filter(x=>x.result&&(x.result.includes('성립')||x.result.includes('합의'))).length;
      const ratio2=Math.round(won/total*100);
      const totalClaim=consumerCases.reduce((a,x)=>a+(x.claimAmount||0),0);
      const totalResult=consumerCases.reduce((a,x)=>a+(x.amount||0),0);
      return `<div class="card" style="margin-top:0;">
        <h3>소비자원 전체 현황</h3>
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;">
          <div class="sc"><div class="sl">전체 건수</div><div class="sv bl">${total}건</div></div>
          <div class="sc"><div class="sl">인용률</div><div class="sv ${ratio2>=50?'gr':'rd'}">${ratio2}%</div></div>
...
function deleteCA(id){
  consumerCases=consumerCases.filter(x=>x.id!==id);
  persist();
  nav('consumer',document.querySelector('.nb[data-fn="navConsumer"]'));
}
