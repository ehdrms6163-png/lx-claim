...
function renderDash(){
  ...
  const byS={접수:0,검토:0,처리:0,종결:0,면책:0,보류:0};
  ...
  const active=tot-(byS['종결']||0)-(byS['면책']||0);
  $('stat-grid').innerHTML=`
  ...
  // 상태별 차트
  const sc={접수:'#378ADD',검토:'#EF9F27',처리:'#7F77DD',종결:'#639922',면책:'#888',보류:'#E24B4A'};
      ...
      <div class="card"><h3>EP 설치이력</h3>
        ...
        <div style="margin-top:9px;display:flex;gap:5px;align-items:center;">
          ...
          <select id="st-sel" style="padding:4px 7px;font-size:12px;border:0.5px solid var(--bd2);border-radius:var(--r-md);background:var(--bg1);color:var(--tx1);font-family:var(--font);">
            ${['접수','검토','처리','종결','면책','보류'].map(s=>`<option ${c.status===s?'selected':''}>${s}</option>`).join('')}
          </select>
