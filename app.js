...
function _initHandlers(){
  ...
  // 증권 한도 저장 버튼
  const bsp=$('btn-save-policy');if(bsp)bsp.addEventListener('click',savePolicySettings);
  // 설정탭 로드 시 기존 값 복원
  if(policySettings.length){const ps=policySettings[0];const y=$('cfg-policy-year');if(y)y.value=ps.year||'';const h=$('cfg-policy-ha');if(h)h.value=ps.haLimit||'';const a=$('cfg-policy-ac');if(a)a.value=ps.acLimit||'';}
  // 상태 재계산 버튼
...
function renderLinkedCard({claimId, minorId, caId, suitId}={}){
  ...
  const linkedCA=caId?null  // 소비자원 상세에서는 자기 자신 제외
    :consumerCases.find(x=>(claimId&&x.claimRef===claimId)||(minorId&&x.minorRef===minorId)||(linkedMinor&&x.minorRef===linkedMinor.id))||null;
  const linkedSuits=suitId?[]  // 소송 상세에서는 자기 자신 제외
...
/* ══════════════════════════════════════════════
   증권별 손해율
══════════════════════════════════════════════ */
function getPolicyYear(date){
  if(!date)return null;
  const d=new Date(date);
  const y=d.getFullYear();
  const m=d.getMonth()+1;
  return m>=4?y:y-1; // 4월 이전이면 전년도 증권
}
function getPolicyClaims(startYear){
...
function renderPolicyStats(){
  ...
  const ps=policySettings[0];
  const year=ps.year||new Date().getFullYear();
  const filtered=getPolicyClaims(year);
  // 가정설치 증권
  const haClaims=filtered.filter(c=>c.groupCode==='HA');
  const haPaid=haClaims.reduce((a,c)=>a+(c.finalPayment||0),0);
  const haLimit=ps.haLimit||0;
  const haRatio=haLimit>0?Math.round(haPaid/haLimit*100):null;
  // 에어컨+정수기/빌트인 증권
  const acClaims=filtered.filter(c=>['ARN','ARR','WB'].includes(c.groupCode));
  const acPaid=acClaims.reduce((a,c)=>a+(c.finalPayment||0),0);
  const acLimit=ps.acLimit||0;
  const acRatio=acLimit>0?Math.round(acPaid/acLimit*100):null;
  el.innerHTML=`
    <div style="grid-column:span 4;font-size:12px;font-weight:600;color:var(--tx2);padding-bottom:4px;border-bottom:0.5px solid var(--bd3);">
      증권 손해율 — ${year}.04.01 ~ ${year+1}.03.31
    </div>
    <div class="sc"><div class="sl">가정설치 지급보험금</div><div class="sv bl">${fmt만원(haPaid)}</div></div>
    <div class="sc"><div class="sl">가정설치 한도</div><div class="sv">${haLimit?fmt만원(haLimit):'-'}</div></div>
    <div class="sc"><div class="sl">가정설치 손해율</div><div class="sv ${haRatio!==null?(haRatio>=80?'rd':'gr'):''}"><b>${haRatio!==null?haRatio+'%':'-'}</b></div></div>
    <div class="sc"><div class="sl">가정설치 건수</div><div class="sv">${haClaims.length}건</div></div>
    <div class="sc"><div class="sl">에어컨/정수기 지급</div><div class="sv bl">${fmt만원(acPaid)}</div></div>
    <div class="sc"><div class="sl">에어컨/정수기 한도</div><div class="sv">${acLimit?fmt만원(acLimit):'-'}</div></div>
    <div class="sc"><div class="sl">에어컨/정수기 손해율</div><div class="sv ${acRatio!==null?(acRatio>=80?'rd':'gr'):''}"><b>${acRatio!==null?acRatio+'%':'-'}</b></div></div>
    <div class="sc"><div class="sl">에어컨/정수기 건수</div><div class="sv">${acClaims.length}건</div></div>`;
}
function savePolicySettings(){
  const year=Number(($('cfg-policy-year')||{}).value)||new Date().getFullYear();
  const haLimit=Number(($('cfg-policy-ha')||{}).value)||0;
  const acLimit=Number(($('cfg-policy-ac')||{}).value)||0;
  if(policySettings.length)policySettings[0]={year,haLimit,acLimit};
  else policySettings.push({year,haLimit,acLimit});
  persist();
  const msg=document.createElement('div');
  msg.style.cssText='position:fixed;bottom:24px;right:24px;padding:12px 18px;background:var(--green);color:#fff;border-radius:10px;font-size:13px;font-weight:500;z-index:400;';
  msg.textContent='✓ 증권 한도 저장 완료';
...
function openCADetail(id){
  ...
  $('ca-detail-content').innerHTML=`
    ...
    ${renderLinkedCard({claimId:c.claimRef||null, minorId:c.minorRef||null})}`;
...
function openSuitDetail(id){
  ...
  $('suit-detail-content').innerHTML=`
    ...
    ${renderLinkedCard({claimId:s.claimRef||null})}`;
  ...
  if(stageSel)stageSel.addEventListener('change',()=>{
    ...
    const inputStyle='width:100%;padding:5px 8px;font-size:12px;border:0.5px solid var(--bd2);border-radius:var(--r-md);background:var(--bg1);color:var(--tx1);font-family:var(--font);margin-bottom:5px;box-sizing:border-box;';
    const fields={
      '소장접수':`<input type="date" id="sf-date" style="${inputStyle}"><input type="text" id="sf-route" placeholder="접수경로 (예: 법무팀 ***책임 통해 메일로 전달)" style="${inputStyle}"><input type="text" id="sf-note" placeholder="추가 내용" style="${inputStyle}">`,
      '보험접수':`<input type="date" id="sf-date" style="${inputStyle}"><input type="text" id="sf-note" placeholder="보험사명, 접수번호 등" style="${inputStyle}">`,
      '피제소대응품의':`<input type="date" id="sf-date" style="${inputStyle}"><input type="text" id="sf-note" placeholder="품의 내용" style="${inputStyle}">`,
      '인장날인':`<input type="date" id="sf-date" style="${inputStyle}"><input type="text" id="sf-firm" placeholder="법무법인명 (예: 법무법인 **)" style="${inputStyle}"><input type="text" id="sf-note" placeholder="추가 내용" style="${inputStyle}">`,
      '변론':`<input type="date" id="sf-date" style="${inputStyle}"><input type="text" id="sf-court" placeholder="법원 및 법정 (예: 서울지법 202호 법정)" style="${inputStyle}"><input type="text" id="sf-time" placeholder="시간 (예: 14:00)" style="${inputStyle}"><input type="text" id="sf-note" placeholder="변론 내용" style="${inputStyle}">`,
      '판결':`<input type="date" id="sf-date" style="${inputStyle}">
        <select id="sf-result" style="${inputStyle}"><option value="">판결결과 선택</option><option>승소</option><option>일부승소</option><option>패소</option></select>
        <input type="number" id="sf-amount" placeholder="판결금액 (원)" style="${inputStyle}">
        <input type="number" id="sf-ratio" placeholder="승소율 (%)" style="${inputStyle}">
        <input type="text" id="sf-dept" placeholder="담당재판부" style="${inputStyle}">
        <input type="text" id="sf-note" placeholder="판결 요지" style="${inputStyle}">`,
      '조정':`<input type="date" id="sf-date" style="${inputStyle}">
        <select id="sf-result" style="${inputStyle}"><option value="">조정결과 선택</option><option>조정성립</option><option>조정불성립</option></select>
        <input type="number" id="sf-amount" placeholder="조정금액 (원)" style="${inputStyle}">
        <input type="number" id="sf-ratio" placeholder="소가 대비 조정율 (%)" style="${inputStyle}">
        <input type="text" id="sf-note" placeholder="조정 내용" style="${inputStyle}">`,
      '항소':`<input type="date" id="sf-date" style="${inputStyle}">
        <select id="sf-appellant" style="${inputStyle}"><option value="">항소인 선택</option><option>원고(당사)</option><option>피고(상대방)</option></select>
        <input type="text" id="sf-court" placeholder="항소법원" style="${inputStyle}">
        <input type="text" id="sf-note" placeholder="항소 이유 요약" style="${inputStyle}">`,
    };
    sf.innerHTML=fields[stage]||'';
  });
