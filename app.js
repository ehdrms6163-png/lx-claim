...
/* saveSuit 오버라이드 — 소송 저장 시 연결된 클레임 상태 소송중으로 */
const _origSaveSuit=saveSuit;
function saveSuit(){
  _origSaveSuit();
  // 클레임ref가 있으면 상태 업데이트
  const claimRef=($('suit-f-claimref')||{}).value||window._pendingSuitClaimId||'';
  if(claimRef){
    const c=claims.find(x=>x.id===claimRef);
    if(c&&c.status!=='종결'&&c.status!=='면책'){c.status='소송중';persist();}
  }
