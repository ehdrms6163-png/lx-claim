...
function saveCA(){
  ...
  if(curCAId){
    ...
    if(!d.history)d.history=[{date:new Date().toISOString().slice(0,10),text:`${d.type} 접수`}];
...
function openCADetail(id){
  ...
  $('ca-detail-content').innerHTML=`
    ...
    ${renderLinkedCard({caId:c.id, claimId:c.claimRef||null, minorId:c.minorRef||null})}`;
...
function openSuitDetail(id){
  ...
  $('suit-detail-content').innerHTML=`
    ...
    ${renderLinkedCard({suitId:s.id, claimId:s.claimRef||null})}`;
