...
function openMinorDetail(id){
  ...
  $('minor-detail-content').innerHTML=`
    ...
    <div class="dg">
      ...
      <div class="card">
        ...
        <h3>처리 이력</h3>
        <div>${(m.history||[]).map(h=>`<div class="tl-i"><div class="tl-d"></div><div class="tl-ln"></div><div><div>${h.text}</div><div class="tl-dt">${h.date}</div></div></div>`).join('')}</div>
        <div style="display:flex;gap:7px;margin-top:9px;">
  ...
  if(bah)bah.addEventListener('click',()=>{
    ...
    m.history.push({date:new Date().toISOString().slice(0,10),text:inp.value.trim()});
...
function linkMinorToCA(minorId){
  ...
  const prefill={
    ...
    type:'자율조정',status:'접수',
    history:[
      {stage:`소액클레임 ${m.id} 이관`,date:new Date().toISOString().slice(0,10)},
      ...(m.history||[]).map(h=>({...h})),
      {stage:'자율조정 접수',date:new Date().toISOString().slice(0,10)},
      {date:new Date().toISOString().slice(0,10),text:'소비자원 자율조정 접수'},
    ],
  };
  // 소비자원 탭으로 이동 후 모달 열기
  document.querySelectorAll('.sec').forEach(s=>s.classList.remove('active'));
  ...
  history.forEach((h,i)=>{
    if(h.stage){
      ...
      html+=`<div class="tl-i" style="align-items:flex-start;">
        ...
        <div class="tl-ln"></div>
        <div style="flex:1;">
          <div id="hist-text-${i}" style="font-size:13px;">${h.text||''}</div>
...
function bindHistoryEditable(history, containerSelector, onUpdate){
  ...
  container.querySelectorAll('[data-hist-edit]').forEach(btn=>{
    btn.addEventListener('click',()=>{
      ...
      const newText=prompt('이력 내용 수정:', h.text||'');
      if(newText===null)return;
      const newDate=prompt('날짜 수정 (YYYY-MM-DD):', h.date||'');
      if(newDate===null)return;
      h.text=newText.trim();
      h.date=newDate.trim()||h.date;
      onUpdate();
    });
  });
  // 삭제 버튼
