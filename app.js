...
function _initHandlers(){
  ...
  const suitesave=$('btn-suit-edit-save');if(suitesave)suitesave.addEventListener('click',saveSuitEdit);if(mnback)mnback.addEventListener('click',()=>nav('minor',document.querySelector('.nb[data-fn="navMinor"]')));
...
function openDetail(id){
  ...
  $('detail-content').innerHTML=`
    ...
    ${(()=>{
      // 연결된 소액클레임 이력 표시
      const linked=minorClaims.find(m=>m.claimRef===c.id);
      if(!linked)return '';
      return `<div class="card" style="margin-bottom:13px;background:#EAF3DE;border-color:#C0DD97;">
        <h3 style="color:var(--green);">소액클레임 연결 이력 (${linked.id})</h3>
        <div style="font-size:12px;color:var(--tx2);margin-bottom:8px;">${linked.name} / ${linked.adate||linked.idate||'-'} / ${linked.type||'-'}</div>
        ${(linked.history||[]).map(h=>`<div class="tl-i"><div class="tl-d" style="background:var(--green);"></div><div class="tl-ln"></div><div><div style="font-size:12px;">${h.text}</div><div class="tl-dt">${h.date}</div></div></div>`).join('')}
      </div>`;
    })()}
    ${(()=>{
...
function openCADetail(id){
  ...
  function renderHistory(history){
    ...
    if(cur.items.length||cur.label)groups.push(cur);
    return groups.map(g=>`
      ${g.label?`<div style="display:flex;align-items:center;gap:6px;margin:10px 0 6px;"><div style="height:1px;flex:1;background:var(--bd3);"></div><span style="font-size:11px;font-weight:600;color:var(--blue);white-space:nowrap;padding:0 6px;">${g.label}</span><div style="height:1px;flex:1;background:var(--bd3);"></div></div>`:''}
      ${g.items.map(h=>`<div class="tl-i"><div class="tl-d"></div><div class="tl-ln"></div><div><div style="font-size:13px;">${h.text}</div><div class="tl-dt">${h.date}</div></div></div>`).join('')}
    `).join('');
  }
