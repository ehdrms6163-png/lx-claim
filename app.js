  ...
  if(brs)brs.addEventListener('click',()=>{
    ...
    claims.forEach(c=>{
      ...
      if(row&&row.처리구분){
        ...
        if(proc.includes('종결')||proc.includes('지급완료')||proc.includes('면책')||proc.includes('부지급'))newStatus='종결';
...
function autoCreateClaims(newRows, insId){
  ...
  newRows.forEach(r=>{
    ...
    const proc=r.처리구분||'';
    let claimStatus='접수';
    if(proc.includes('종결')||proc.includes('지급완료')||proc.includes('면책')||proc.includes('부지급'))claimStatus='종결';
