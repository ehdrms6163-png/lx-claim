...
function openMinorDetail(id){
  ...
  $('minor-detail-content').innerHTML=`
    ...
    ${renderLinkedCard({minorId:m.id, claimId:m.claimRef||null})}`;
...
function renderLinkedCard({claimId, minorId, caId, suitId}={}){
  // 보험클레임
  const linkedClaim=claimId?claims.find(x=>x.id===claimId):null;
  // 소액클레임 - minorId 직접 또는 claimRef로 찾기
  const linkedMinor=minorId?minorClaims.find(x=>x.id===minorId)
    :claimId?minorClaims.find(x=>x.claimRef===claimId):null;
  // 소비자원 - caId는 자기 자신이므로 제외, claimRef 또는 minorRef로 찾기
  const linkedCA=caId?null
    :consumerCases.find(x=>{
  ...
  // 소송 - suitId는 자기 자신이므로 제외, claimRef로 찾기
  const linkedSuits=suitId?[]
    :claimId?lawsuits.filter(x=>x.claimRef===claimId)
    :minorId?lawsuits.filter(x=>{const m=minorClaims.find(mc=>mc.id===minorId);return m&&m.claimRef&&x.claimRef===m.claimRef;})
    :[];

  const parts=[];

  if(linkedClaim)parts.push(`
...
function saveCAEdit(){
  ...
  c.no=$('cae-f-no').value;
  c.type=$('cae-f-type').value;
  c.claimant=$('cae-f-claimant').value;
  c.phone=$('cae-f-phone').value;
  c.respondent=$('cae-f-respondent').value;
  c.date=$('cae-f-date').value;
  c.status=$('cae-f-status').value;
  c.assignee=($('cae-f-asgn')||{}).value||'';
  c.claimAmount=Number($('cae-f-claim-amount').value)||0;
  c.amount=Number($('cae-f-amount').value)||0;
  c.claimRef=$('cae-f-claimref').value;
  c.minorRef=$('cae-f-minorref').value;
  c.committeeDate=$('cae-f-committee-date').value;
  c.committeePlace=$('cae-f-committee-place').value;
  c.committeeDate2=$('cae-f-committee-date2').value;
  c.committeePlace2=$('cae-f-committee-place2').value;
  c.result=$('cae-f-result').value;
  c.desc=$('cae-f-desc').value;
  c.note=$('cae-f-note').value;
  c.updatedAt=new Date().toISOString().slice(0,10);
  persist();
  renderCAStats();renderCA();
  openCADetail(c.id);
}
  ...
  $('ca-detail-content').innerHTML=`
    ...
    <div class="dg">
      <div class="card">
        ...
        <div style="border-top:0.5px solid var(--bd3);margin:9px 0;"></div>
        <h3>금액</h3>
        <div class="dr"><span class="dk">청구금액</span><span>${claimAmt?fmt만원(claimAmt):'-'}</span></div>
