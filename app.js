...
function renderGlobalSearch(){
  ...
  if(!q){el.innerHTML='<div class="empty">주문번호, 고객명, 설치기사명을 입력하세요</div>';return;}
  const lq=q.toLowerCase();
  const matchedClaims=claims.filter(c=>
    (c.name||'').toLowerCase().includes(lq)||
    (c.tname||'').toLowerCase().includes(lq)||
    (c.orderNo||'').toLowerCase().includes(lq)||
    (c.product||'').toLowerCase().includes(lq)||
    (c.addr||'').toLowerCase().includes(lq)||
    (c.id||'').toLowerCase().includes(lq)||
    (c.insNo||'').toLowerCase().includes(lq)
  );
  const matchedMinor=minorClaims.filter(c=>
    (c.name||'').toLowerCase().includes(lq)||
    (c.tech||'').toLowerCase().includes(lq)||
    (c.order||'').toLowerCase().includes(lq)||
    (c.addr||'').toLowerCase().includes(lq)||
    (c.id||'').toLowerCase().includes(lq)
  );
  const matchedEP=epRecs.filter(r=>
    (r.고객명||'').toLowerCase().includes(lq)||
    (r.설치기사||'').toLowerCase().includes(lq)||
    (r.주문번호||r.설치주문번호||'').toLowerCase().includes(lq)||
    (r.주소||'').toLowerCase().includes(lq)
  ).slice(0,10);
  if(!matchedClaims.length&&!matchedMinor.length&&!matchedEP.length){
  ...
  function linkedBadges(linked,claimRef){
    const parts=[];
    if(claimRef)parts.push(`<span style="font-size:11px;padding:2px 7px;background:#EAF3DE;color:var(--green);border-radius:99px;">보험클레임 ${claimRef}</span>`);
    if(linked.minor)parts.push(`<span style="font-size:11px;padding:2px 7px;background:#EAF3DE;color:var(--green);border-radius:99px;">소액 ${linked.minor.id}</span>`);
    if(linked.ca)parts.push(`<span style="font-size:11px;padding:2px 7px;background:#E6F1FB;color:var(--blue);border-radius:99px;">소비자원 ${linked.ca.type} ${linked.ca.no||''}</span>`);
    if(linked.suit)parts.push(`<span style="font-size:11px;padding:2px 7px;background:#FCEBEB;color:var(--red);border-radius:99px;">소송 ${linked.suit.no||''}</span>`);
  ...
  let html='';
  if(matchedEP.length){
    html+=`<div style="font-size:13px;font-weight:600;color:var(--tx2);margin-bottom:8px;">EP 설치이력 (${matchedEP.length}건)</div>
    <div class="card" style="margin-bottom:14px;padding:0;overflow:hidden;">
      <div class="th" style="grid-template-columns:130px 1fr 100px 80px 80px;"><span>주문번호</span><span>고객명/주소</span><span>설치기사</span><span>설치일</span><span>신규접수</span></div>
      ${matchedEP.map(r=>`<div class="tr" style="grid-template-columns:130px 1fr 100px 80px 80px;">
        <span style="font-family:var(--mono);font-size:11px;">${r.주문번호||r.설치주문번호||'-'}</span>
        <span><b style="font-weight:500;">${r.고객명||'-'}</b> <span style="color:var(--tx2);font-size:11px;">${(r.주소||'').slice(0,25)}</span></span>
        <span style="font-size:12px;">${r.설치기사||'-'}</span>
  ...
  if(matchedClaims.length){
    html+=`<div style="font-size:13px;font-weight:600;color:var(--tx2);margin-bottom:8px;">보험 클레임 (${matchedClaims.length}건)</div>`;
    matchedClaims.forEach(c=>{
      const linked=getLinked(c.id,null);
      html+=`<div class="card" style="margin-bottom:10px;">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
          <span style="font-family:var(--mono);font-size:11px;color:var(--blue);cursor:pointer;" data-claim-id="${c.id}">${c.id}</span>
          <span class="bdg b-${c.status}">${c.status}</span>
          <span style="font-size:12px;color:var(--tx2);">${c.type}</span>
          <span style="flex:1;font-weight:500;">${c.name||'-'}</span>
          <span style="font-size:11px;color:var(--tx2);">${c.insDate||c.date||'-'}</span>
        </div>
        <div style="font-size:12px;color:var(--tx2);">${c.addr||''} ${c.tname?'· 기사: '+c.tname:''} ${c.orderNo?'· 주문: '+c.orderNo:''}</div>
        ...
        <div style="display:flex;gap:6px;margin-top:8px;padding-top:6px;border-top:0.5px solid var(--bd3);">
          ...
          <button class="btn sm" data-mail-insurance="${c.id}">📧 보험사 PL접수</button>
        </div>
      </div>`;
    });
  }
  if(matchedMinor.length){
    html+=`<div style="font-size:13px;font-weight:600;color:var(--tx2);margin-bottom:8px;">소액클레임 (${matchedMinor.length}건)</div>`;
    matchedMinor.forEach(m=>{
      const linked=getLinked(m.claimRef,m.id);
      html+=`<div class="card" style="margin-bottom:10px;">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
          <span style="font-family:var(--mono);font-size:11px;color:var(--blue);cursor:pointer;" data-minor-id="${m.id}">${m.id}</span>
          <span class="bdg b-접수">${m.status}</span>
          <span style="font-size:12px;color:var(--tx2);">${m.type||'-'}</span>
          <span style="flex:1;font-weight:500;">${m.name||'-'}</span>
          <span style="font-size:11px;color:var(--tx2);">${m.adate||m.date||'-'}</span>
        </div>
        <div style="font-size:12px;color:var(--tx2);">${m.addr||''} ${m.tech?'· 기사: '+m.tech:''} ${m.order?'· 주문: '+m.order:''} ${m.logistics?'· '+m.logistics:''}</div>
...
function saveCA(){
  ...
  if(curCAId){
    ...
    if(i>=0){d.history=consumerCases[i].history||[];consumerCases[i]=d;}
  }else{
...
function openCADetail(id){
  ...
  $('ca-detail-content').innerHTML=`
    ...
    <div class="dg">
      ...
      <div class="card">
        ...
        <div style="border-top:0.5px solid var(--bd3);margin:12px 0;"></div>
        <h3>전체 처리 이력</h3>
        <div id="ca-history-list">${renderHistory(c.history)}</div>
        <div style="display:flex;gap:7px;margin-top:9px;">
  ...
  if(bah)bah.addEventListener('click',()=>{
    ...
    c.history.push({date:new Date().toISOString().slice(0,10),text:inp.value.trim()});
...
function openSuitDetail(id){
  ...
  $('suit-detail-content').innerHTML=`
    ...
    <div class="dg">
      ...
      <div class="card">
        ...
        <div style="border-top:0.5px solid var(--bd3);margin:12px 0;"></div>
        <h3>처리 이력</h3>
        <div>${(s.history||[]).map(h=>`<div class="tl-i"><div class="tl-d"></div><div class="tl-ln"></div><div><div style="font-size:13px;">${h.text}</div><div class="tl-dt">${h.date}</div></div></div>`).join('')||'<div style="font-size:12px;color:var(--tx3);padding:4px 0;">이력 없음</div>'}</div>
        <div style="display:flex;gap:7px;margin-top:9px;">
          <input type="text" id="suit-hist-inp" placeholder="처리 내용 (기일결과, 준비서면 등)..." style="flex:1;padding:5px 8px;font-size:12px;border:0.5px solid var(--bd2);border-radius:var(--r-md);background:var(--bg1);color:var(--tx1);font-family:var(--font);">
  ...
  if(bah)bah.addEventListener('click',()=>{
    ...
    s.history.push({date:new Date().toISOString().slice(0,10),text:inp.value.trim()});
